...
int successfull_task_req(  const char *method,int value,int msg_type)
{
  
    //prom_counter_add(manager_metrics->metrics_est_request,(double)value,0);
    //prom_counter_inc(manager_metrics->metrics_est_request,(const char*[]){"/metrics",method,"success"});
    //printf("successful_task_completed\n");
    switch(msg_type)
    {
    	case 1:prom_counter_add(manager_metrics->metrics_est_request,(double)value,0); 
	...
	case 4: prom_counter_inc(manager_metrics->metrics_delete_request,0);
    		break;
	default :printf("defult option=%d\n",msg_type);
		break;    
    }
}
int deletionfull_task_req(  const char *method,int msg_type)
{
 ...
 switch(msg_type)
    {
    	case 5://prom_counter_add(manager_metrics->metrics_est_responce,(double)value,0); 
		prom_counter_inc(manager_metrics->metrics_est_request,0);
    		break;
	case 6: prom_counter_inc(manager_metrics->metrics_mod_responce,0);
    		break;	
	case 7: prom_counter_inc(manager_metrics->metrics_report_responce,0);
    		break;
	case 8: prom_counter_inc(manager_metrics->metrics_delete_responce,0);
    		break;
	default :printf("defult option=%d\n",msg_type);
		break;    
    }
...
int metrics_test(char *config)
{
	...
	if( pfcpObj)
	{
        ...
        app_config__set_str( pfcpObj, "IPv4", promo_config->pfcp_IPv4, 16);
   ...
   metrics_init();
	...
	printf("function return in daemo\n");
    return 1;
}
