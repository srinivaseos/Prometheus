...
static const struct hpack_index *  	hpack_table_getbyid( long, struct hpack_index *, struct hpack_table *);
static const struct hpack_index *  	hpack_table_getbyheader( struct hpack_header *, struct hpack_index *, struct hpack_table *);
static int	 						hpack_table_add(struct hpack_header *, struct hpack_table *);
...
static long	 						hpack_decode_index(struct hbuf *, unsigned char, const struct hpack_index **, struct hpack_table *);
static int	 						hpack_decode_literal(struct hbuf *, unsigned char, struct hpack_table *);
static int	 						hpack_encode_int(struct hbuf *, long, unsigned char, unsigned char);
static int	 						hpack_encode_str(struct hbuf *, char *);

static int	 						hpack_huffman_init(void);
static struct 						hpack_huffman_node * hpack_huffman_new(void);
static void	 						hpack_huffman_free(struct hpack_huffman_node *);

static struct hbuf *				hbuf_new(unsigned char *, size_t);
...
static unsigned char *		 		hbuf_release(struct hbuf *, size_t *);
...
static void hpack_huffman_free( struct hpack_huffman_node * root)
{
	if (root == NULL)
		return;
	...
	for (i = 0; i < HPACK_HUFFMAN_SIZE; i++) 
	{
		...
		for (j = hph->hph_length; j > 0; j--) 
		{
			if ((hph->hph_code >> (j - 1)) & 1) 
			{
				if (cur->hpn_one == NULL) 
				{
					if ((node = hpack_huffman_new()) == NULL)
						goto fail;
...
struct hpack_header * hpack_header_new( void)
{
	return (calloc(1, sizeof(struct hpack_header)));
}

void hpack_header_free(struct hpack_header * hdr)
{
	if (hdr == NULL)
		return;
...
struct hpack_header * hpack_header_add( struct hpack_headerblock * hdrs, const char * name, const char * value, enum hpack_header_index index)
{
...
struct hpack_headerblock * hpack_headerblock_new( void)
{
...
void hpack_headerblock_free( struct hpack_headerblock * hdrs)
{
	struct hpack_header	* hdr;
...
struct hpack_table * hpack_table_new( size_t max_table_size)
{
	...
	hpack->htb_max_table_size = hpack->htb_table_size = max_table_size == 0 ? HPACK_MAX_TABLE_SIZE : max_table_size;
...
void hpack_table_free( struct hpack_table * hpack)
{
	if (hpack == NULL)
		return;
...
static const struct hpack_index * hpack_table_getbyid( long index, struct hpack_index * idbuf, struct hpack_table * hpack)
{
	...
	if (index <= dynidx) 
	{
		...
		if (id->hpi_id != index)
			errx(1, "corrupted HPACK static table %ld != %ld", id->hpi_id, index);
	} 
	else 
	{
		/* Dynamic table */
		TAILQ_FOREACH_REVERSE(hdr, hpack->htb_dynamic, hpack_headerblock, hdr_entry) 
		{
			dynidx++;
			if (dynidx == index) 
			{
...
static const struct hpack_index * hpack_table_getbyheader( struct hpack_header * key, struct hpack_index * idbuf, struct hpack_table * hpack)
{
	...
	for (i = 0; i < dynidx; i++) 
	{
		...
		if ((id->hpi_value != NULL && key->hdr_value != NULL) && strcasecmp(id->hpi_value, key->hdr_value) == 0)
			return (id);
	}
	...
	TAILQ_FOREACH_REVERSE( hdr, hpack->htb_dynamic, hpack_headerblock, hdr_entry) 
	{
		...
		if ((hdr->hdr_value != NULL && key->hdr_value != NULL) && strcasecmp(hdr->hdr_value, key->hdr_value) == 0) 
		{
			idbuf->hpi_id = dynidx;
			idbuf->hpi_name = hdr->hdr_name;
			idbuf->hpi_value = hdr->hdr_value;
...
static int hpack_table_evict( long size, long newsize, struct hpack_table * hpack)
{
	struct hpack_header	*hdr;

	while (size < (hpack->htb_dynamic_size + newsize) && (hdr = TAILQ_FIRST(hpack->htb_dynamic)) != NULL) 
	{
		TAILQ_REMOVE(hpack->htb_dynamic, hdr, hdr_entry);
		hpack->htb_dynamic_entries--;
		hpack->htb_dynamic_size -= strlen(hdr->hdr_name) + strlen(hdr->hdr_value) + 32;
		hpack_header_free(hdr);
	}

	if (TAILQ_EMPTY(hpack->htb_dynamic) && hpack->htb_dynamic_entries != 0 && hpack->htb_dynamic_size != 0)
...
static int hpack_table_add( struct hpack_header * hdr, struct hpack_table * hpack)
{
	long		 newsize;
	...
	if (newsize > hpack->htb_table_size) 
	{
		...
		hpack_table_evict( hpack->htb_table_size, newsize, hpack);

	if (hpack_header_add(hpack->htb_dynamic, hdr->hdr_name, hdr->hdr_value, hdr->hdr_index) == NULL)
		return (-1);
...
static int hpack_table_setsize( long size, struct hpack_table * hpack)
{
...
size_t hpack_table_size(struct hpack_table *hpack)
{
	return ((size_t)hpack->htb_dynamic_size);
}


static int hbuf_readchar(struct hbuf *buf, unsigned char *c)
{
...
static int hbuf_readbuf(struct hbuf *buf, unsigned char **ptr, size_t len)
{
...
static int hbuf_advance(struct hbuf *buf, size_t len)
{
	if (buf->rpos + len > buf->size)
		return (-1);
	buf->rpos += len;
...
static size_t hbuf_left( struct hbuf * buf)
{
...
static void hbuf_free(struct hbuf *buf)
{
	if (buf == NULL)
		return;
...
static struct hbuf * hbuf_new(unsigned char *data, size_t len)
{
	struct hbuf	* buf;
	size_t		 size = len;
...
static long hpack_decode_int( struct hbuf *buf, unsigned char prefix)
{
	unsigned long	 i = 0;
	unsigned char	 b = 0, m;
...
static long hpack_decode_index( struct hbuf * buf, unsigned char prefix, const struct hpack_index ** idptr, struct hpack_table * hpack)
{
	...
	if (hasvalue && (hdr->hdr_value = strdup(id->hpi_value)) == NULL) 
	{
		free(hdr->hdr_name);
		hdr->hdr_name = NULL;
		return (-1);
...
static char * hpack_decode_str(struct hbuf *buf, unsigned char prefix)
{
	long i;
	unsigned char * ptr, c;
	char * str;
	...
	if (hbuf_readbuf(buf, &ptr, (size_t)i) == -1 || hbuf_advance(buf, (size_t)i) == -1)
		return (NULL);
...
static int hpack_decode_literal( struct hbuf * buf, unsigned char prefix, struct hpack_table * hpack)
{
	struct hpack_header * hdr = hpack->htb_next;
	const struct hpack_index *id;
	long i;
...
static int hpack_decode_buf( struct hbuf * buf, struct hpack_table * hpack)
{
	struct hpack_header	*hdr = NULL;
	unsigned char		 c;
	long			 i;
	...
	if ((c & HPACK_M_INDEX) == HPACK_F_INDEX) 
	{
		...
		/* 7 bit index */
		if ((i = hpack_decode_index( buf, HPACK_M_INDEX, NULL, hpack)) == -1)
			goto fail;
	...
	else if ((c & HPACK_M_LITERAL_INDEX) == HPACK_F_LITERAL_INDEX) 
	{
		...
		/* 6 bit index */
		if (hpack_decode_literal( buf, HPACK_M_LITERAL_INDEX, hpack) == -1)
			goto fail;
	...
	else if ((c & HPACK_M_LITERAL_NO_INDEX) == HPACK_F_LITERAL_NO_INDEX) 
	{
		...
		/* 4 bit index */
		if (hpack_decode_literal(buf, HPACK_M_LITERAL_NO_INDEX, hpack) == -1)
			goto fail;
	}
	...
	else if ((c & HPACK_M_TABLE_SIZE_UPDATE) == HPACK_F_TABLE_SIZE_UPDATE) 
	{
		...
		/* 5 bit index */
		if ((i = hpack_decode_int(buf, HPACK_M_TABLE_SIZE_UPDATE)) == -1)
			goto fail;
...
static unsigned char * hbuf_release( struct hbuf * buf, size_t *len)
{
	unsigned char	*data;
	...
	if (buf->wpos != buf->size) 
	{
		...
		if ((data = reallocarray(buf->data, buf->wpos, 1)) == NULL) 
		{
			hbuf_free(buf);
			return (NULL);
		}
...
static int hbuf_realloc(struct hbuf *buf, size_t len)
{
	unsigned char * ptr;
	size_t newsize;
	...
	if ((ptr = reallocarray(buf->data, newsize, 1)) == NULL)
		return (-1);
...
static int hbuf_writechar(struct hbuf *buf, unsigned char c)
{
	return (hbuf_writebuf(buf, &c, 1));
}


static int hbuf_writebuf(struct hbuf *buf, unsigned char *data, size_t len)
{
	if ((buf->wpos + len > buf->size) && hbuf_realloc(buf, len) == -1)
		return (-1);
...
unsigned char * hpack_huffman_encode( unsigned char * data, size_t len, size_t * encoded_len)
{
	struct hbuf * hbuf;
	struct hpack_huffman * hph;
...
unsigned char * hpack_huffman_decode( unsigned char * buf, size_t len, size_t * decoded_len)
{
	...
	for (i = 0; i < len; i++) 
	{
		...
		for (j = 8; j > 0; j--) 
		{
			...
			if (hbuf_writechar( hbuf, (unsigned char)node->hpn_sym) == -1) 
			{
				printf("%s: failed to add '%c'\n", __func__, node->hpn_sym);
				goto fail;
			}
			node = root;
		}
...
char * hpack_huffman_decode_str( unsigned char * buf, size_t len)
{
	unsigned char	*data;
	char		*str;
	size_t		 data_len;
	...
	if ((str = reallocarray( data, data_len + 1, 1)) == NULL) 
	{
...
static int hpack_encode_int( struct hbuf * buf, long i, unsigned char prefix, unsigned char type)
{
	unsigned char	b, m;
...
static int hpack_encode_str(struct hbuf *buf, char *str)
{
	...
	if (len > 0 && len < slen) 
	{
		printf("%s: encoded huffman code (size %ld, from %ld)\n", __func__, len, slen);
		
		if (hpack_encode_int(buf, len, HPACK_M_LITERAL, HPACK_F_LITERAL_HUFFMAN) == -1)
			goto done;
	...
	{
		if (hpack_encode_int(buf, slen, HPACK_M_LITERAL, HPACK_F_LITERAL) == -1)
			goto done;
...
struct hpack_headerblock * hpack_decode( unsigned char * data, size_t len, struct hpack_table * hpack)
{
...
unsigned char * hpack_encode( struct hpack_headerblock * hdrs, size_t * encoded_len, struct hpack_table * hpack)
{
	const struct hpack_index * id;
	struct hpack_index idbuf;
	struct hpack_table * ctx = NULL;
	...
	{
		printf("%s: header %s: %s (index %d)\n", __func__, hdr->hdr_name, hdr->hdr_value == NULL ? "(null)" : hdr->hdr_value, hdr->hdr_index);
		...
		{
			printf("%s: index %zu (%s: %s)\n", __func__, id->hpi_id, id->hpi_name, id->hpi_value == NULL ? "(null)" : id->hpi_value);
			
			if (hpack_encode_int(hbuf, id->hpi_id, HPACK_M_INDEX, HPACK_F_INDEX) == -1)
				goto fail;
		...
		{
			printf("%s: index+name %zu, %s\n", __func__, id->hpi_id, hdr->hdr_value);

			if (hpack_encode_int(hbuf, id->hpi_id, mask, flag) == -1)
				goto fail;
		} 
		else 
		{
			printf("%s: literal %s: %s\n", __func__, hdr->hdr_name, hdr->hdr_value);
