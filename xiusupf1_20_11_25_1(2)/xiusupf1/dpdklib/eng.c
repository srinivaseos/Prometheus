...
// updated 03-FEB-2025  ip-flow / dedicated session
// updated 07-FEB-2025  priv
...
void * util_malloc(size_t const size, void * const user_ptr)
{
    (void)user_ptr;
	return (void *)rte_malloc("dpi_malloc", size, 0);
}

void * util_realloc(void * const ptr, size_t const size, void * const user_ptr)
{
    (void)user_ptr;
	return (void *)rte_realloc(ptr, size, 0);
}

void util_free(void * const ptr, void * const user_ptr)
{
    (void)user_ptr;
	rte_free(ptr);
}
...
int __dpdk_env__find_full_numa_path_exits( int i)
{
	...
	{
		printf( "2MB numa node path not found=%s\n", folder);
	}
	
	memset( folder, 0, sizeof(folder));
	sprintf( folder, "/sys/devices/system/node/node%d/hugepages/hugepages-1048576kB/", i);	
	...
	{
		printf( "1GB numa node path not found=%s\n", folder);
	}
...
void __dpdk_env__set_hugepages( int i, int numa, int sz)
{
	char folder[1024];
	memset( folder, 0, sizeof(folder));
	...
	if( numa == 1) 
	{
		...
		{
			sprintf( folder, "echo 16 > /sys/devices/system/node/node%d/hugepages/hugepages-1048576kB/nr_hugepages", i);
		}
	} 
	else 
	{
		...
		{
			sprintf( folder, "echo 16 > /sys/kernel/mm/hugepages/hugepages-1048576kB/nr_hugepages");
		}
	}
...
// ../../dpdk/dpdk-stable-21.11.2/usertools/dpdk-hugepages.py -s
int __dpdk_env__create_huge_pages()
{
	struct stat sb;
	...
	if (stat(folder, &sb) == 0 && S_ISDIR(sb.st_mode)) 
	{
		printf("hugepages: \"%s\" - folder exists\n", folder);
		return 0;
	} 
	else 
	{
		...
		printf("hugepages: \"%s\" - not exists., creating \"%s\"\n", folder, folder);
...
void __dpdk_env__configure_hugepages_for_nodes()
{
	if( __dpdk_env__is_numa() == 1)
	{
		...
		while(1)
		{
			...
			if( exists == 1)
			{
				...
				if( fexists)
				{
					__dpdk_env__set_hugepages( i, 1, fexists);
				}
	...
	{
		__dpdk_env__set_hugepages( 0, 0, 2);
	}
...
typedef struct dpdk_interface_queue
{
...
} dpdk_interface_queue_t;


typedef struct dpe_dpi_ins
{
	uint32_t index;
} dpe_dpi_ins_t;
...
typedef struct dpdk_interface
{
	...
	uint32_t 	RxDescLimit;
	uint32_t 	TxDescLimit;
	...
	uint16_t	receive_queue[10];
	uint16_t	send_queue[10];
	dpdk_interface_queue_t if_queue[50];
	...
	uint32_t	DestIP;
	uint8_t		DstMac[8];
	uint8_t		DstMacReceived;
	uint64_t    DstMacTime;
	
	uint32_t	TargetIP;
	
	uint32_t	CQosEnabled;
	uint32_t	QosEnabled;
	...
	uint64_t	sched_enqueed;
	uint64_t	sched_dequeed;
	uint64_t	sched_dropped;
	uint64_t	sched_forwarded;

	uint64_t	last_sched_enqueed;
	uint64_t	last_sched_dequeed;
	uint64_t	last_sched_dropped;
	uint64_t	last_sched_forwarded;
...
typedef struct privdata {
	...
	int rgid;
	uint8_t * flowptr;
	dpdk_interface_t * srcInterface;
	dpdk_interface_t * dstInterface;
	uint8_t * iph;
	uint16_t  iphlen;
	uint32_t  ueip;
	uint32_t  EnablePktLogs;
} privdata_t;
...
#define DPE__MAX_DEBUG_UPIP				30


typedef struct dpdk_pkt_logs 
{
	int Enabled;
	uint32_t ueip[DPE__MAX_DEBUG_UPIP];
	uint32_t ueip_count;

	uint32_t teid[DPE__MAX_DEBUG_UPIP];
	uint32_t teid_count;
} dpdk_pkt_logs_t;
...
typedef struct dpdk_engine 
{
	...
	int IPv4HTCount;
	int IPv6HTCount;
	struct rte_hash * IPv4HT;
	struct rte_hash * IPv6HT;
	...
	uint32_t debug_ueip[DPE__MAX_DEBUG_UPIP];
	uint32_t debug_ueip_count;

	uint32_t EnableSelfLoadTest;
	uint32_t StartSelfLoadTest;
	...
	int 			DPICoreCount;
	dpe_dpi_ins_t 	dpe_dpi_ins[40];
	//struct rte_mempool * dpi_mempool;
	struct rte_ring    * dpi_ring[40];

	
	uint32_t	EnableAllUELogs;
...
uint32_t dpe__pktlogs__log_for_teid( uint32_t teid)
{
	int i = 0;
	
	for( i = 0; i < __dpe->PktLogs.teid_count; i++)
	{
		if( __dpe->PktLogs.teid[i] == teid)
		{
			return 1;
		}
	}		
...
uint32_t dpe__pktlogs__log_for_ueipv4( uint32_t ueip)
{
	int i = 0;
	
	for( i = 0; i < __dpe->PktLogs.ueip_count; i++)
	{
		if( __dpe->PktLogs.ueip[i] == ueip)
		{
			return 1;
		}
	}		
...
void __dpdk_env__start__vfio_pci()
{
	...
	sprintf( folder, "sudo modprobe %s", __dpe->pci_driver);
	sts = system( folder);
	printf( "[%s] - command status = %d errno=%d|%s\n", folder, sts, errno, strerror(errno));

	if( strcmp( __dpe->pci_driver, "vfio-pci") == 0)
	{
		memset( folder, 0, sizeof(folder));
	...
	printf( "\n");
	printf( "wating for 5 seconds for vfio-pci to initalize\n");
	printf( "\n");
	sleep(5);
}
...
void __dpdk_env__configure__pci( char * dpdk_abs_path, char * ifname, char * bus_info)
{
	...
	memset( folder, 0, sizeof(folder));
	sprintf( folder, "sudo %sdpdk-devbind.py --bind=%s %s", dpdk_abs_path, __dpe->pci_driver, bus_info);
	sts = system( folder);
	printf( "[%s] - command status = %d  |%d\n", folder, sts, __LINE__);
	//  errno=%d|%s  , errno, strerror(errno)

	printf("UPF-EAL:  dpdk-bind command executed for eth=%s  bus_info=%s  |%d\n", ifname, bus_info, __LINE__);
	usleep( 999999);
}
...
void dpdk__initexit()
{
	...
	// printf("3 dpdk exiting %d\n", __LINE__);
	
	// // rte_eal_cleanup();
	// printf("4 dpdk exiting %d  end\n", __LINE__);
}
...
void dpe__nat__delete_user_nat( uint32_t ueip)
{
	if( __dpe->userNAT)
	{
...
dpe_nat_entry_t * dpe__nat__find_or_create_natentry( dpdk_interface_t * interface, uint32_t ueip, uint32_t dstip, uint16_t srcport, uint16_t dstport, uint8_t protocol)
{
	if( __dpe->userNAT)
	{
		uint8_t key[6];
...
dpe_nat_entry_t * dpe__nat__find_natentry_by_natsrcport( dpdk_interface_t * interface, uint32_t pkt_dstip, uint16_t pkt_dstport, uint32_t pkt_srcip, uint16_t pkt_srcport, uint8_t protocol)
{
...
void dpe__udp_client_message( uint8_t * Data, int tIndex)
{
	...
	if( cmd->size == len)
	{
		...
		{
			pfcp_stack___exec_cmd( cmd->cmd, cmd->operation, cmd->interface, cmd->command);
		}
		else if( cmd->cmd == 4)
		{
			int i = 0;
			uint32_t ip = htonl(inet_addr( cmd->command));
			
			// printf("received pkt_log command for ip=%s|%u|%u  op=%u  %d\n", cmd->command, 
				// ip, htonl(ip), cmd->operation, __LINE__);

			
			if( cmd->operation == 1)
			{
				for( i = 0; i < DPE__MAX_DEBUG_UPIP; i++)
				{
					if( __dpe->debug_ueip[i] == 0)
					{
						__dpe->debug_ueip[i] = ip;
						__dpe->debug_ueip_count++;
						break;
					}
				}
			}
			...
			{
				for( i = 0; i < __dpe->debug_ueip_count; i++)
				{
					if( __dpe->debug_ueip[i] == ip)
					{
						__dpe->debug_ueip[i] = 0;
						__dpe->debug_ueip_count--;
						break;
					}
				}
		...
		{
			__dpe->StartSelfLoadTest = 1;
		}
		else if( cmd->cmd == 6)
		{
			// printf( "cmd=%d  seid=%d  rg=%d  uplink=%d  downlink=%d\n", 
					// cmd->cmd, cmd->operation, cmd->interface, cmd->uplink, cmd->downlink);
			cp__usage( cmd->operation, cmd->interface, cmd->uplink, cmd->downlink); 		
		}
		else if( cmd->cmd == 7)
		{
			cp__quota( cmd->operation);
		}
		else if( cmd->cmd == 8)
		{
			//printf( "cmd->operation=%d\n", cmd->operation);
			if( cmd->operation == 0 || cmd->operation == 1)
			{
				cp__enable_quota_logs( cmd->operation);
			}
		}
		else if( cmd->cmd == 9)
		{
			...
			if( cmd->operation == 0 || cmd->operation == 1)
			{
				__dpe->EnablePktLogs = cmd->operation;
			}
		}
...
uint32_t dpe__pkt_log_enabled( uint32_t ip)
{
	if( __dpe->EnableAllUELogs == 1)
		return 1;
...
{
	//printf( "rte_ipv6_hdr sz=%ld  %s|%d\n", sizeof(struct rte_ipv6_hdr), __FUNCTION__, __LINE__); == 40 confirmed
	
	//printf( "sizeof(struct rte_gtp_hdr)=%lu\n", sizeof(struct rte_gtp_hdr));
	...
	else
	{
		...
		__dpe->enable_pcap_on__gi_gi	= 0;
		__dpe->access_mode 				= 0;
		__dpe->EnableSelfLoadTest		= 0;
		__dpe->StartSelfLoadTest		= 0;
		...
		__dpe->EnablePktLogs 			= dpdk_config__get_int2( config, "EnablePktLogs", 0, 1);
		__dpe->EnableAllUELogs 			= dpdk_config__get_int2( config, "EnableAllUELogs", 0, 1);
		...
		__dpe->ExecEnvSetup				= dpdk_config__get_int2( config, "ExecEnvSetup", 0, 0);
		__dpe->IPv4HTCount				= dpdk_config__get_int2( config, "IPv4HTCount", 0, 0);
		__dpe->IPv6HTCount				= dpdk_config__get_int2( config, "IPv6HTCount", 0, 0);
		__dpe->DPICoreCount				= dpdk_config__get_int2( config, "DPICoreCount", 0, 0);
		__dpe->EnableSelfLoadTest		= dpdk_config__get_int2( config, "EnableSelfLoadTest", 0, 0);
		
		printf("EnablePktLogs=%u\n", 			__dpe->EnablePktLogs);
		...
		printf("IPv4HTCount=%u\n", 				__dpe->IPv4HTCount);
		printf("IPv6HTCount=%u\n", 				__dpe->IPv6HTCount);
		printf("IPv6HTCount=%u\n", 				__dpe->EnableSelfLoadTest);
		...
		json_t * jPCIDriver = json_object_get( config, "PCIDriver");

		if( jPCIDriver)
		{
			char * tempPCIDriver  = (char *)json_string_value( jPCIDriver);
			int  tempPCIDriverLen = json_string_length( jPCIDriver);

			if( tempPCIDriverLen > 0)
			{
				memcpy( __dpe->pci_driver, tempPCIDriver, tempPCIDriverLen);
				printf( "PCIDriver=%s\n", __dpe->pci_driver);
			}
		}
		else
		{
			strcpy( __dpe->pci_driver, "vfio-pci");
		}
		...
		json_t * jpacketLogs = json_object_get( config, "PacketLogs");
		
		if( jpacketLogs)
		{
			if( dpdk_config__get_int2( jpacketLogs, "Enabled", 0, 1) == 1)
			{
				__dpe->PktLogs.Enabled = 1;
				__dpe->PktLogs.ueip_count = 0;
				__dpe->PktLogs.teid_count = 0;
				
				json_t * plUEIPs = json_object_get( jpacketLogs, "UE-IPs");
				...
				if( plUEIPs)
				{
					if( json_is_array( plUEIPs))
					{
						count = json_array_size( plUEIPs);
						
						for( i = 0; i < count; i++)
						{
							if( __dpe->PktLogs.ueip_count < DPE__MAX_DEBUG_UPIP)
							{
								item = json_array_get( plUEIPs, i);
								ctemp  = (char *)json_string_value( item);
								
								__dpe->PktLogs.ueip[ __dpe->PktLogs.ueip_count] = inet_addr( ctemp);
								__dpe->PktLogs.ueip_count++;
								
								dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "added ip=%s|%u to packet debug list", ctemp, __dpe->PktLogs.ueip[ __dpe->PktLogs.ueip_count - 1]);
								printf( "added ip=%s|%u to packet debug list\n", ctemp, __dpe->PktLogs.ueip[ __dpe->PktLogs.ueip_count - 1]);
							}
						}
					}
				}
				...
				json_t * pTEIDs = json_object_get( jpacketLogs, "TEIDs");
				
				if( pTEIDs)
				{
					if( json_is_array( pTEIDs))
					{
						count = json_array_size( pTEIDs);
						
						for( i = 0; i < count; i++)
						{
							if( __dpe->PktLogs.teid_count < DPE__MAX_DEBUG_UPIP)
							{
								item = json_array_get( pTEIDs, i);
								
								__dpe->PktLogs.teid[ __dpe->PktLogs.teid_count] = json_integer_value( item);
								__dpe->PktLogs.teid_count++;
								
								dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "added teid=%u to packet debug list", __dpe->PktLogs.teid[ __dpe->PktLogs.teid_count - 1]);
								printf( "added teid=%u to packet debug list\n", __dpe->PktLogs.teid[ __dpe->PktLogs.teid_count - 1]);
							}
						}
					}
				}
		...
		int port_count = 2000;	// we will not do NAT
				...
				else
				{
					...
					for( i = 0; i < count; i++)
					{
						...
						if( interface)
						{
							...
							interfaceObject->CQosEnabled		= 0;
							interfaceObject->RxDescLimit		= 256;
							interfaceObject->TxDescLimit		= 256;
							...
							int iqid = 0;
							for( iqid = 0; iqid < 10; iqid++)
							{
								interfaceObject->receive_queue[iqid] = iqid;
								interfaceObject->send_queue[iqid] = iqid;
							}
							
							
							json_t * jRQI = json_object_get( interface, "RQI");
							json_t * qitem = NULL;
							
							int receive_queue_index = 0;
							int send_queue_index = 0;
							
							if(jRQI)
							{
								if( json_is_array(jRQI))
								{
									int jRQICount = json_array_size( jRQI);
									
									for( iqid = 0; iqid < jRQICount; iqid++)
									{
										qitem = json_array_get( jRQI, iqid);
										interfaceObject->receive_queue[iqid] = json_integer_value( qitem);
										receive_queue_index++;
									}
								}
							}
							...
							json_t * jSQI = json_object_get( interface, "SQI");
							if(jSQI)
							{
								if( json_is_array(jSQI))
								{
									int jRQICount = json_array_size( jSQI);
									
									for( iqid = 0; iqid < jRQICount; iqid++)
									{
										qitem = json_array_get( jSQI, iqid);
										interfaceObject->send_queue[iqid] = json_integer_value( qitem);
										send_queue_index++;
									}
								}
							}
							...
							interfaceObject->RxDescLimit = dpdk_config__get_int( interface, "RxDescLimit", 256, 256);
							interfaceObject->TxDescLimit = dpdk_config__get_int( interface, "TxDescLimit", 256, 256);
							...
							if( receive_queue_index > 0 &&  interfaceObject->RecvQueue != receive_queue_index)
							{
								printf("Index=%d  MISMATCH  receive_queue_index=%d RecvQueue=%d \n", i, receive_queue_index, interfaceObject->RecvQueue);
								dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "Index=%d  MISMATCH  receive_queue_index=% RecvQueue=%d \n", i, receive_queue_index, interfaceObject->RecvQueue);
								exit(0);								
							}
							...
							if( send_queue_index > 0 &&  interfaceObject->SendQueue != send_queue_index)
							{
								printf("Index=%d  MISMATCH  send_queue_index=%d SendQueue=%d \n", i, send_queue_index, interfaceObject->SendQueue);
								dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "Index=%d  MISMATCH  send_queue_index=% SendQueue=%d \n", i, send_queue_index, interfaceObject->SendQueue);
								exit(0);								
							}
							...
							interfaceObject->RecvWorkerCountPerQ = 1;
							...
							if(jDestIP)
							{
								...
								interfaceObject->DstMacReceived = 0;
								interfaceObject->DstMacTime = (uint64_t)time(NULL);
							...
							interfaceObject->CQosEnabled = dpdk_config__get_int( interface, "EnableCQos", 0, 0);
							//interfaceObject->QosEnabled = dpdk_config__get_int( interface, "EnableQoS", 0, 0);
							interfaceObject->QosEnabled = 0;
							printf( "QosEnabled=%u CQosEnabled=%u   %u\n", interfaceObject->QosEnabled, interfaceObject->CQosEnabled, __LINE__);
							...
							{	
								...
								/*
									for CGNAT More ports required, a single IP cannot serve
										add bunch of IP to accomodate it
								*/ 
		...
		if( iNATEntryCount > 0)
		{
			...
			if( interface && 1 == 2)
			{
				...
				if( nat_ip)
				{
					...
					sleep(1);
					dpe__nat__delete_user_nat( user_ip);
					dpe__user_nat___print( user_ip);
					printf("\n\n");
...
int dpe__get_workercorecount()
{
	...
	while( interface)
	{
		ccount += interface->RecvWorkerCount;
		interface = interface->Next;
	}
...
{
	int ccount 					= __dpe->DPICoreCount;
	int rccount 				= 0;	
	int qos_core_count 			= 1;
	int ipflows_del_core_count 	= 1;
	...
	while( interface)
	{
		...
		if( (interface->QosEnabled == 1 || interface->CQosEnabled == 1) && ptrPacketGeneratorFunction > 0)
		{
			ccount = interface->RecvQueue + interface->SendQueue + (interface->RecvWorkerCount * interface->RecvWorkerCountPerQ) + interface->SendWorkerCount + qos_core_count + ipflows_del_core_count + interface->PacketGeneratorCount;
			
			dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "core count for interface=%s core-count=%d RecvQueue=%d SendQueue=%d Workers=%d qos=%d pgen=%d", 
					interface->Port, ccount, 
					interface->RecvQueue, interface->SendQueue, 
					(interface->RecvWorkerCount * interface->RecvWorkerCountPerQ), interface->SendWorkerCount, 
					qos_core_count, interface->PacketGeneratorCount
				);
		}
		else if( (interface->QosEnabled == 1 || interface->CQosEnabled == 1) && !(ptrPacketGeneratorFunction > 0))
		{
			ccount = interface->RecvQueue + interface->SendQueue + (interface->RecvWorkerCount * interface->RecvWorkerCountPerQ) + interface->SendWorkerCount + qos_core_count + ipflows_del_core_count;

			dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "core count for interface=%s core-count=%d RecvQueue=%d SendQueue=%d Workers=%d qos=%d", 
					interface->Port, ccount, 
					interface->RecvQueue, interface->SendQueue, 
					(interface->RecvWorkerCount * interface->RecvWorkerCountPerQ), interface->SendWorkerCount, 
					qos_core_count
		...
		{
			ccount = interface->RecvQueue + interface->SendQueue + (interface->RecvWorkerCount * interface->RecvWorkerCountPerQ) + interface->SendWorkerCount + interface->PacketGeneratorCount + ipflows_del_core_count;

			dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "core count for interface=%s core-count=%d RecvQueue=%d SendQueue=%d Workers=%d pgen=%d", 
					interface->Port, ccount, 
					interface->RecvQueue, interface->SendQueue, 
					(interface->RecvWorkerCount * interface->RecvWorkerCountPerQ), interface->SendWorkerCount, 
					interface->PacketGeneratorCount
				);
		}
		else
		{
			...
			dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "core count for interface=%s core-count=%d RecvQueue=%d SendQueue=%d Workers=%d", 
					interface->Port, ccount, 
					interface->RecvQueue, interface->SendQueue, 
					(interface->RecvWorkerCount * interface->RecvWorkerCountPerQ), interface->SendWorkerCount
				);
	...
	dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "total required cores=%d as per configuration for dpdk", rccount);
	return (rccount + 1);
}
...
struct rte_mbuf * dpe__create_gtp_echo_response( struct rte_mbuf * mbuf, dpdk_interface_t * interface, uint32_t teid)
{
	struct rte_mbuf * resp_m_buf = rte_pktmbuf_alloc( interface->mempool[0]);
	...
	if( resp_m_buf)
	{
		...
		if( teid > 0) {
			ran_teid = cp__get_ran_teid( teid);
		}
		...
		res_buffer[43] = 0x02;
		*(uint32_t*)&res_buffer[46] = rte_cpu_to_be_32( ran_teid);
...
int dpe__isarpformyip( struct rte_mbuf * mbuf_req, uint32_t myip, dpdk_interface_t * interface, dpdk_nat_ip_t * natIP)
{
	...
	if( request_arp->arp_opcode == 512 && request_arp->arp_data.arp_tip == myip /*(Reply to myRequest)*/)
	{
		...
		if( request_arp->arp_data.arp_sip == interface->DestIP)
		{
			...
			memcpy( interface->DstMac, request_arp->arp_data.arp_sha.addr_bytes, 6);		
			interface->DstMacReceived = 1;
			interface->DstMacTime = (uint64_t)time(NULL);
		}
		else
		{
...
uint16_t dpe__process_forward( dpdk_interface_t * interface, struct rte_mbuf ** pkts, uint16_t recv_pkts, struct rte_mbuf ** processed_pkts, struct rte_mbuf ** dropped_mbufs, uint16_t * dropped_cnt_out, struct rte_mbuf ** arp_mbufs, uint16_t * arp_cnt_out, struct rte_mbuf ** gtp_mbufs, uint16_t * gtp_cnt_out, int recv_worker_index, uint16_t worker_queue_id, int rci)
{
	struct rte_ether_hdr * eth = NULL;
	uint16_t dropped_cnt = 0;
	uint16_t arp_cnt = 0;
	...
	uint16_t seqn_no = 0;
	uint8_t extlen = 0;
	uint8_t ext_itemlen = 0;
	uint8_t ext_loopcount = 0;
	char * ext_gtp_pkt = NULL;
	int usedbytes = 0;
	...
	uint16_t iph_header_len = 0;
	uint16_t root_iph_header_len = 0;

	uint16_t ipv4_fragment_offset = 0;

	struct rte_mbuf * fpkt = NULL;
	uint32_t fpkt_len = 0;
	
	char * plptr = NULL;
	uint32_t toTrim = 0;
	
	struct rte_ipv4_hdr * froot_ip_hdr 	= NULL;
	
	struct rte_ipv6_hdr * fue_ip4_hdr 	= NULL;
	struct rte_ipv6_hdr * fue_ip6_hdr 	= NULL;
	struct rte_udp_hdr * froot_udp 		= NULL;


	struct rte_tcp_hdr * fue_tcp = NULL;
	struct rte_udp_hdr * fue_udp = NULL;
	
	uint32_t fip_tcp_len = 0;
	uint32_t ue_tcp_len = 0;
	uint32_t ue_pl_len = 0;
	struct rte_ipv4_hdr * __ue_ip_hdr = NULL;
	struct rte_ipv6_hdr * __ue_ip6_hdr = NULL;
	int rgid = 0;
	
	privdata_t * pdata = NULL;
	uint8_t * flowPtr = NULL;
			...
			case 8:		// RTE_ETHER_TYPE_IPV4:
				{
					if( interface->ProcessType == PROCESS_TYPE__GTP_Gi)
					{
						...
						root_iph_header_len = root_ip_hdr->ihl * 4;

						if( root_ip_hdr->next_proto_id == IPPROTO_ICMP && root_ip_hdr->dst_addr == interface->IPv4)
						{
							//icmp_buf = ((uint8_t*)root_ip_hdr + 20);				// ihl 05112024
							icmp_buf = ((uint8_t*)root_ip_hdr + root_iph_header_len);
						...
						//udp = (struct rte_udp_hdr *)((unsigned char *)root_ip_hdr + 20);			// ihl 05112024
						udp = (struct rte_udp_hdr *)((unsigned char *)root_ip_hdr + root_iph_header_len);
						gtp = (struct rte_gtp_hdr *)((unsigned char *)udp + sizeof(struct rte_udp_hdr));
							...
							case 1:		// ECHO REQUEST
								{
									...
									gtp_mbufs[gtp_cnt] = dpe__create_gtp_echo_response( pkts[i], interface, teid);
						...
						*/
						...
						if( __dpe->SkipAPICalls == 0) 
						{
							...
							{
								if( __dpe->EnablePktLogs == 1)
								{
									dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "GTP Packet received from RAN[%u.%u.%u.%u] with TEID=%u, session not found hence dropped", 
										ran_ip & 0xFF, (ran_ip >> 8) & 0xFF, (ran_ip >> 16) & 0xFF, (ran_ip >> 24) & 0xFF, teid);
								}
								
								if( __dpe->PktLogs.Enabled == 1 && __dpe->EnablePktLogs == 0)
								{
									if( dpe__pktlogs__log_for_teid(teid))
									{
										dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "GTP Packet received from RAN[%u.%u.%u.%u] with TEID=%u, session not found hence dropped", 
											ran_ip & 0xFF, (ran_ip >> 8) & 0xFF, (ran_ip >> 16) & 0xFF, (ran_ip >> 24) & 0xFF, teid);
									}
							...
							// else if( __dpe->EnablePktLogs == 1 && sess)
							// {
						...
						if( __dpe->EnablePktLogs == 1)
						{
							dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "GTP Packet received from RAN[%u.%u.%u.%u] with TEID=%u, session found and allowed", 
								ran_ip & 0xFF, (ran_ip >> 8) & 0xFF, (ran_ip >> 16) & 0xFF, (ran_ip >> 24) & 0xFF, teid);							
						}
						...
						*/
						//usedbytes = 50;
						usedbytes = ( 14 + root_iph_header_len + 8 + 8);	// == 50
						extlen = 0;
						
						//printf("is_seqn_present=%d, gtp_ext =%d\n ", is_seqn_present, gtp_ext);
						if( is_seqn_present == 1 || gtp_ext == 4)
						{
							//usedbytes = 58;
							extlen = 0;
							
							if( is_seqn_present == 1 && gtp_ext == 0)
							{
								// sizeof(struct rte_gtp_hdr)=8
								
								usedbytes = ( 14 + root_iph_header_len + 8 + 12);	//
								payload = (((unsigned char *)gtp) + 12);			// only legth of gtp header
							}
							else if( gtp_ext == 4)
							{
								ext_itemlen = 0;
								ext_gtp_pkt = (char*)(((unsigned char *)gtp) + 11);
								ext_loopcount = 0;
								
								//printf( "ext_gtp_pkt[%02X]\n", ext_gtp_pkt[0] & 0xFF);
								//exit(0);


								while(ext_gtp_pkt)
								{
									ext_itemlen = ((uint8_t)ext_gtp_pkt[1]) * 4;
									//printf( "ext_itemlen=%d  next-ext[%02X]  len=%02X\n", ext_itemlen, ext_gtp_pkt[ext_itemlen] & 0xFF, ext_gtp_pkt[1] & 0xFF);
									// exit(0);
									
									extlen += ext_itemlen;
									
									
									//printf("hasNextExt=%u\n", (uint8_t)*((uint8_t*)&ext_gtp_pkt[ext_itemlen]));
									
									if( (*(uint8_t*)&ext_gtp_pkt[ext_itemlen]) == 0)
									{
										ext_gtp_pkt = NULL;
										//printf("No Next Extension\n");
										break;
									}
									else
									{
										//printf("Next Extension Found\n");
										ext_gtp_pkt = &ext_gtp_pkt[ext_itemlen];
										//break;
										
										//printf( "-->  %02X  %02X  %02X\n", ext_gtp_pkt[0] & 0xFF, ext_gtp_pkt[1] & 0xFF, ext_gtp_pkt[2] & 0xFF);
								...
								usedbytes = ( 14 + root_iph_header_len + 8 + (12 + extlen));	//
								payload = (((unsigned char *)gtp) + (12 + extlen));				// only legth of gtp header
							}

							//printf( "Total GTP Header Length = %d, only extlength=%d\n", 12 + extlen, extlen);


							// pcap_dumper_t * dumper = dpdk_pkt__open_pcap_dump( "./gtp_packet_04012025_01.pcap");
							// dpdk_pkt__write_pcap( dumper, pkts[i]);
							// dpdk_pkt__close_pcap( dumper);
							// exit(0); 
						...
						{
							//sizeof(struct rte_gtp_hdr)=8
							usedbytes = ( 14 + root_iph_header_len + 8 + (8));
						...
						if(!payload)
						{
							if( __dpe->EnablePktLogs == 1)
							{
								dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "Payload Not Found for GTP Packet RAN[%u.%u.%u.%u] TEID=%u", 
									ran_ip & 0xFF, (ran_ip >> 8) & 0xFF, (ran_ip >> 16) & 0xFF, (ran_ip >> 24) & 0xFF, teid);
							}
						...
						if(((payload[0] & 0xF0) >> 4) == 4)
						{
							...
							ipv4_fragment_offset = ue_ip_hdr->fragment_offset;
							
							//ue_proto_packet = payload + sizeof(struct rte_ipv4_hdr);			// ihl 05112024					
							ue_proto_packet = payload + (ue_ip_hdr->ihl * 4);
						}
						else
						{
						...
						if( ue_proto == IPPROTO_UDP)
						{
							...
							// 26-JUNE-2025 TEMPORARY
							// if( (ipv4_fragment_offset & 0x2000) == 0x2000 || (ipv4_fragment_offset & 0x1FFF) > 0 )
							// {
								// ue_src_port = 0;
								// ue_dst_port = 0;
							// }
						...
						{
							ue_src_port = *(uint16_t*)&ue_proto_packet[4];
							ue_dst_port = *(uint16_t*)&ue_proto_packet[6];
						}
						else 
						{
							ue_src_port = *(uint16_t*)&ue_proto_packet[0];
						...
						if( __dpe->EnablePktLogs == 1)
						{	
							dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "GTP Packet info RAN[%u.%u.%u.%u]  ue_ip=%u.%u.%u.%u  dst_ip=%u.%u.%u.%u  src_port=%u  dst_port=%u  protocol=%u  with TEID=%u", 
						...
						// printf( "ran_ip=%u teid=%u sess=%p  ue_ipv=%u  usedbytes=%u\n", ran_ip, teid, sess, ue_ipv, usedbytes);
						// exit(0);
						...
						if( __dpe->SkipAPICalls == 0)
						{
							if( ue_ipv == 4)
							{
								if( cp__flow_allowed_sess_v4( sess, ue_dst_ip, ue_src_port, ue_dst_port, ue_proto, p, pkts[i]->pkt_len, PROCESS_TYPE__GTP_Gi, recv_worker_index, &rgid, rci, &flowPtr) == 0)
								{
									if( __dpe->EnablePktLogs == 1)
									{
							...
							{
								eth->ether_type = rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV6);											//IPv6
								
								if( cp__flow_allowed_sess_v6( sess, ue_dst_ip6, ue_src_port, ue_dst_port, ue_proto, p, pkts[i]->pkt_len, PROCESS_TYPE__GTP_Gi, recv_worker_index, &rgid, rci, &flowPtr) == 0)
								{
									if( __dpe->EnablePktLogs == 1)
									{
										dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "GTP Packet received from RAN[%u.%u.%u.%u]  ue_ip=%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X  dst_ip=%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X  src_port=%u  dst_port=%u  protocol=%u  with TEID=%u and dropped", 
											ran_ip & 0xFF, (ran_ip >> 8) & 0xFF, (ran_ip >> 16) & 0xFF, (ran_ip >> 24) & 0xFF, 
											ue_src_ip6[8] & 0xFF, ue_src_ip6[9] & 0xFF, ue_src_ip6[10] & 0xFF, ue_src_ip6[11] & 0xFF, ue_src_ip6[12] & 0xFF, ue_src_ip6[13] & 0xFF, ue_src_ip6[14] & 0xFF, ue_src_ip6[15] & 0xFF,
											ue_dst_ip6[8] & 0xFF, ue_dst_ip6[9] & 0xFF, ue_dst_ip6[10] & 0xFF, ue_dst_ip6[11] & 0xFF, ue_dst_ip6[12] & 0xFF, ue_dst_ip6[13] & 0xFF, ue_dst_ip6[14] & 0xFF, ue_dst_ip6[15] & 0xFF,
											ue_src_port, ue_dst_port, ue_proto,
											teid);
									}
								...
								{
									dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "GTP Packet received from RAN[%u.%u.%u.%u]  ue_ip=%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X  dst_ip=%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X  src_port=%u  dst_port=%u  protocol=%u  with TEID=%u and allowed", 
										ran_ip & 0xFF, (ran_ip >> 8) & 0xFF, (ran_ip >> 16) & 0xFF, (ran_ip >> 24) & 0xFF, 
											ue_src_ip6[8] & 0xFF, ue_src_ip6[9] & 0xFF, ue_src_ip6[10] & 0xFF, ue_src_ip6[11] & 0xFF, ue_src_ip6[12] & 0xFF, ue_src_ip6[13] & 0xFF, ue_src_ip6[14] & 0xFF, ue_src_ip6[15] & 0xFF,
											ue_dst_ip6[8] & 0xFF, ue_dst_ip6[9] & 0xFF, ue_dst_ip6[10] & 0xFF, ue_dst_ip6[11] & 0xFF, ue_dst_ip6[12] & 0xFF, ue_dst_ip6[13] & 0xFF, ue_dst_ip6[14] & 0xFF, ue_dst_ip6[15] & 0xFF,										
											ue_src_port, ue_dst_port, ue_proto,
						...
						pdata = (privdata_t*)rte_mbuf_to_priv( pkts[i]);
						
						if(pdata)
						{
							pdata->userdata1 		= sess;
							pdata->flowIndex 		= 0;
							pdata->rgid 			= rgid;
							pdata->srcInterface 	= interface;
							pdata->dstInterface 	= interface->TxInterface;
							pdata->flowptr 			= flowPtr;
							pdata->iph				= (uint8_t*)&p[14]; 
							pdata->iphlen			= pkts[i]->pkt_len - 14;
							pdata->ueip 			= ue_src_ip;
							pdata->EnablePktLogs 	= 0;
						}
						
						if( __dpe->EnablePktLogs > 0) 
						{
							if( ue_ipv == 4)
							{
								uint32_t enabledForUEIP = dpe__pkt_log_enabled( htonl(ue_src_ip));
								
								if( enabledForUEIP == 1) 
								{
									dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "GTP-Gi: moving packet to next stage   - ue_ip=%u.%u.%u.%u  dst_ip=%u.%u.%u.%u  src_port=%u  dst_port=%u  protocol=%u", 
										ue_src_ip & 0xFF, (ue_src_ip >> 8) & 0xFF, (ue_src_ip >> 16) & 0xFF, (ue_src_ip >> 24) & 0xFF, 
										ue_dst_ip & 0xFF, (ue_dst_ip >> 8) & 0xFF, (ue_dst_ip >> 16) & 0xFF, (ue_dst_ip >> 24) & 0xFF,
										ue_src_port, ue_dst_port, ue_proto);
						...
						*/
						...
						// pcap_dumper_t * dumper2 = dpdk_pkt__open_pcap_dump( "./gtp_ipv6_packet_13052024_01.pcap");
						...
						if( __dpe->PktLogs.Enabled == 1 && __dpe->EnablePktLogs == 0)
						{
							if( dpe__pktlogs__log_for_ueipv4(ue_src_ip))
							{
								pdata->EnablePktLogs = 1;
					...
					else if( interface->ProcessType == PROCESS_TYPE__Gi_GTP)
					{
						...
						iph_header_len  = ue_ip_hdr->ihl * 4;
						...
						//ue_proto_packet = (char *)root_ip_hdr + sizeof(struct rte_ipv4_hdr);			// ihl 05112024					
						ue_proto_packet = (char *)root_ip_hdr + iph_header_len;							
						...
						if( ue_proto == IPPROTO_UDP)
						{
							...
							ue_pl_len = pkts[i]->pkt_len - (14 + iph_header_len + 8);
						}
						else if( ue_proto == IPPROTO_TCP)
						{
							ue_tcp = (struct rte_tcp_hdr *)ue_proto_packet;
							
							ue_tcp_len = ( ue_tcp->data_off >> 4) * 4;
							
							ue_src_port = ue_tcp->src_port;
							ue_dst_port = ue_tcp->dst_port;		
							
							ue_pl_len = pkts[i]->pkt_len - (14 + iph_header_len + ue_tcp_len);
						}
						else if( ue_proto == 50)	//  Encapsulating Security Payload
						{
							...
							ue_pl_len = pkts[i]->pkt_len - (14 + iph_header_len);
						}
						else if( ue_proto == 1)
						{
							ue_src_port = *(uint16_t*)&ue_proto_packet[6];
							ue_dst_port = *(uint16_t*)&ue_proto_packet[4];
							
							ue_pl_len = pkts[i]->pkt_len - (14 + iph_header_len);
						}
						else if( ue_proto == 132)
						{
							ue_src_port = *(uint16_t*)&ue_proto_packet[0];
							ue_dst_port = *(uint16_t*)&ue_proto_packet[2];		
						...
						if( ue_proto == IPPROTO_UDP)
						{
							if( ue_src_port == 17408 && ue_dst_port == 17152)
							{
								...
								//dhcp_packet_len = (pkts[i]->pkt_len - ( 14 + 20 + 8));
								dhcp_packet_len = (pkts[i]->pkt_len - ( 14 + iph_header_len + 8));
						...
						if( __dpe->EnablePktLogs == 1)
						{
							dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "Gi Packet received from ue_ip=%u.%u.%u.%u  dst_ip=%u.%u.%u.%u  src_port=%u  dst_port=%u  protocol=%u  p_ran_teid=%u sess=%p  pkt_len=%u  and allowed", 
								ue_dst_ip & 0xFF, (ue_dst_ip >> 8) & 0xFF, (ue_dst_ip >> 16) & 0xFF, (ue_dst_ip >> 24) & 0xFF,
								ue_src_ip & 0xFF, (ue_src_ip >> 8) & 0xFF, (ue_src_ip >> 16) & 0xFF, (ue_src_ip >> 24) & 0xFF,
								ue_src_port, ue_dst_port, ue_proto, p_ran_teid, sess, pkts[i]->pkt_len);
						}
							
						
						if( __dpe->SkipAPICalls == 0)
						{	
							if( cp__flow_allowed_ueipv4( ue_dst_ip, ue_src_ip, ue_dst_port, ue_src_port, ue_proto, p, pkts[i]->pkt_len, &sess, &p_ran_teid, PROCESS_TYPE__Gi_GTP, recv_worker_index, &qfi, &rgid, rci, &flowPtr) == 0)
							{
								if( __dpe->EnablePktLogs == 1)
								{
									dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "Gi Packet received from ue_ip=%u.%u.%u.%u  dst_ip=%u.%u.%u.%u  src_port=%u  dst_port=%u  protocol=%u teid=%u  EnableNAPT=%u  ProcessType=%u  IPv4=%u|%u session-found=%p  and dropped", 
										ue_dst_ip & 0xFF, (ue_dst_ip >> 8) & 0xFF, (ue_dst_ip >> 16) & 0xFF, (ue_dst_ip >> 24) & 0xFF,
										ue_src_ip & 0xFF, (ue_src_ip >> 8) & 0xFF, (ue_src_ip >> 16) & 0xFF, (ue_src_ip >> 24) & 0xFF,
								...
								if( __dpe->PktLogs.Enabled == 1 && __dpe->EnablePktLogs == 0)
								{
									if( dpe__pktlogs__log_for_ueipv4(teid))
									{
										dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "Gi Packet received from ue_ip=%u.%u.%u.%u  dst_ip=%u.%u.%u.%u  src_port=%u  dst_port=%u  protocol=%u teid=%u  EnableNAPT=%u  ProcessType=%u  IPv4=%u|%u session-found=%p  and dropped", 
											ue_dst_ip & 0xFF, (ue_dst_ip >> 8) & 0xFF, (ue_dst_ip >> 16) & 0xFF, (ue_dst_ip >> 24) & 0xFF,
											ue_src_ip & 0xFF, (ue_src_ip >> 8) & 0xFF, (ue_src_ip >> 16) & 0xFF, (ue_src_ip >> 24) & 0xFF,
							...
							if( __dpe->PktLogs.Enabled == 1 && __dpe->EnablePktLogs == 0)
							{
								if( dpe__pktlogs__log_for_ueipv4(ue_dst_ip))
								{
									dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "Gi Packet received from ue_ip=%u.%u.%u.%u  dst_ip=%u.%u.%u.%u  src_port=%u  dst_port=%u  protocol=%u  p_ran_teid=%u sess=%p  pkt_len=%u  and allowed", 
										ue_dst_ip & 0xFF, (ue_dst_ip >> 8) & 0xFF, (ue_dst_ip >> 16) & 0xFF, (ue_dst_ip >> 24) & 0xFF,
										ue_src_ip & 0xFF, (ue_src_ip >> 8) & 0xFF, (ue_src_ip >> 16) & 0xFF, (ue_src_ip >> 24) & 0xFF,
						...
						if( (orig_pkt_len + prependLen) > 1514)
						{
							//printf("buffer exceeded with bytes=%d  l=%d\n", (orig_pkt_len + prependLen) - 1514, __LINE__);
							fpkt = rte_pktmbuf_alloc( interface->mempool[0]);
							...
							fpkt_len = ((orig_pkt_len + prependLen) - 1514);
							//printf( "fpkt_len=%u\n", fpkt_len);
							
							fpkt_len += (14 + 20 + 8 + 16); //tunnel encapulisation  eth+ip+udp+gtp   
							//printf( "fpkt_len=%u\n", fpkt_len);
							
							fpkt_len += 20;			// Packet IP, 20
							...
							// after IP it will be completely treated as transport protocol 
							// else if( ue_proto == 132)
							// {
								// dropped_mbufs[dropped_cnt] = pkts[i];
								// dropped_cnt++;
						...
						if( fpkt)
						{
							//printf("packet len to trim=%d  fpkt_len=%u  l=%d\n", (pkts[i]->pkt_len - 1514), fpkt_len, __LINE__);
							
							
							fpkt->pkt_len  = fpkt_len;
							fpkt->data_len = fpkt_len;
		
							fp = rte_pktmbuf_mtod( fpkt, char *);
							
							pdata = (privdata_t*)rte_mbuf_to_priv( fpkt);
							...
							if(pdata)
							{
								...
								pdata->dstInterface 	= interface->TxInterface;
								pdata->flowptr			= flowPtr;
								pdata->iph				= (uint8_t*)&fp[14];
								pdata->iphlen			= fpkt_len - 14;
								pdata->ueip 			= ue_dst_ip;
								pdata->EnablePktLogs 	= 0;
							}
						
							
							toTrim = pkts[i]->pkt_len - 1514;
							
							plptr = &fp[ fpkt_len - toTrim];	// 58
							
							rte_memcpy( plptr, &p[1514], toTrim);
							
							
							rte_pktmbuf_trim( pkts[i], toTrim);
							orig_pkt_len -= toTrim;
							ue_pl_len -= toTrim;
							...
							// PRIMARY_PACKET_INFO
							__ue_ip_hdr = (struct rte_ipv4_hdr *)&p[58];
							
							
							//printf( "__ue_ip_hdr->total_length=%d\n", htons(__ue_ip_hdr->total_length));
							__ue_ip_hdr->total_length 		= rte_cpu_to_be_16( rte_cpu_to_be_16( __ue_ip_hdr->total_length) - toTrim);
							__ue_ip_hdr->hdr_checksum 		= 0;
							__ue_ip_hdr->hdr_checksum 		= rte_ipv4_cksum( __ue_ip_hdr);

							if( ue_proto == IPPROTO_UDP)
							{
								ue_udp = (struct rte_udp_hdr *)&p[58 + iph_header_len];
								
								//ue_udp->dgram_len 		= rte_cpu_to_be_16( 1436);
								ue_udp->dgram_len 		= rte_cpu_to_be_16( rte_cpu_to_be_16( ue_udp->dgram_len) - toTrim);
								ue_udp->dgram_cksum		= 0;
								ue_udp->dgram_cksum		= rte_ipv4_udptcp_cksum( __ue_ip_hdr, ue_udp);
							}
							else if( ue_proto == IPPROTO_TCP)
							{
								ue_tcp = (struct rte_tcp_hdr *)&p[58 + iph_header_len];
								ue_tcp->cksum = 0;
								ue_tcp->cksum = rte_ipv4_udptcp_cksum( __ue_ip_hdr, ue_tcp);
							}
							// else if( ue_proto == 132)
							// {
							// }
						...
						*/
						...
						if( ran_ipv == 4) 
						{
							...
							if( pdata)
							{
								pdata->iph			= (uint8_t*)ip_hdr;
								pdata->iphlen		= ((orig_pkt_len-14) + prependLen) - 14;
							}
						...
						if( ran_ipv == 4) 
						{
							...
							gtp->plen 				= rte_cpu_to_be_16( ((orig_pkt_len-14) + gtpHeaderLen)-8);
						...
						// printf( "gtp-len=%d  d%s|%d\n", htons( gtp->plen), __FILE__, __LINE__);
						...
						*(uint16_t *)(((unsigned char *)gtp) + 8) = 0;	//seq no
						gtp->gtp_hdr_info 		= 0x34;					// 0x34 No SEQ-NO, 0x36 SEQ-NO
						...
						if( fpkt)
						{
							...
							ip_hdr = (struct rte_ipv4_hdr *)&fp[14];
							
							ip_hdr->total_length 	= rte_cpu_to_be_16( fpkt_len - 14);
							ip_hdr->hdr_checksum 	= 0;
							ip_hdr->hdr_checksum 	= rte_ipv4_cksum( ip_hdr);
							...
							gtp  					= (struct rte_gtp_hdr *) ((unsigned char *)udp + sizeof(struct rte_udp_hdr));
							//gtp->plen 				= rte_cpu_to_be_16( fpkt_len - (14 + 20 + 8));
							gtp->plen 				= rte_cpu_to_be_16( (fpkt_len - (14 + 20 + 8)) - 8);
							
							
							rte_memcpy( &fp[58], root_ip_hdr, iph_header_len);
							
							
							froot_ip_hdr = (struct rte_ipv4_hdr *)&fp[58];
							froot_ip_hdr->total_length 	= rte_cpu_to_be_16( fpkt_len - 58);
							froot_ip_hdr->hdr_checksum 	= 0;
							froot_ip_hdr->hdr_checksum 	= rte_ipv4_cksum( froot_ip_hdr);
							
							
							if( ue_proto == IPPROTO_UDP)
							{
								rte_memcpy( &fp[ 58 + iph_header_len], ue_udp, 8);
								
								fue_udp = (struct rte_udp_hdr *)&fp[ 58 + iph_header_len];
								
								fue_udp->dgram_len 			= rte_cpu_to_be_16( fpkt_len - (58 + iph_header_len));
								fue_udp->dgram_cksum		= 0;
								fue_udp->dgram_cksum		= rte_ipv4_udptcp_cksum( froot_ip_hdr, fue_udp);
								
							}
							else if( ue_proto == IPPROTO_TCP)
							{
								fip_tcp_len = ( ue_tcp->data_off >> 4) * 4;
								//printf( "seq=%u data_offset=%u  fip_tcp_len=%u  ue_pl_len=%u\n", ue_tcp->sent_seq, ue_tcp->data_off & 0x0F >> 4, fip_tcp_len, ue_pl_len);
								rte_memcpy( &fp[ 58 + iph_header_len], ue_tcp, fip_tcp_len);
								
								
								fue_tcp = (struct rte_tcp_hdr *)&fp[ 58 + iph_header_len];

								fue_tcp->sent_seq = htonl( htonl(ue_tcp->sent_seq) + ue_pl_len);
								
								fue_tcp->cksum = 0;
								fue_tcp->cksum = rte_ipv4_udptcp_cksum( froot_ip_hdr, fue_tcp);
							}
							
							udp->dgram_cksum 		= 0;
							udp->dgram_len 			= rte_cpu_to_be_16( fpkt_len - (14 + 20));	// remove udp-len from prependLen
							udp->dgram_cksum		= rte_ipv4_udptcp_cksum( ip_hdr, udp);
						...
						if( __dpe->EnablePktLogs > 0) 
						{
							if( ue_ipv == 4)
							{
								uint32_t enabledForUEIP = dpe__pkt_log_enabled( htonl(ue_dst_ip));
								
								if( enabledForUEIP == 1) 
								{
									dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "Gi-GTP: moving packet to next stage   - ue_ip=%u.%u.%u.%u  dst_ip=%u.%u.%u.%u  src_port=%u  dst_port=%u  protocol=%u", 
										ue_dst_ip & 0xFF, (ue_dst_ip >> 8) & 0xFF, (ue_dst_ip >> 16) & 0xFF, (ue_dst_ip >> 24) & 0xFF,
										ue_src_ip & 0xFF, (ue_src_ip >> 8) & 0xFF, (ue_src_ip >> 16) & 0xFF, (ue_src_ip >> 24) & 0xFF,
										ue_src_port, ue_dst_port, ue_proto);
						...
						// pcap_dumper_t * dumper2 = dpdk_pkt__open_pcap_dump( "./gi_gtp_packet_18NOV2024_1.pcap");
						// dpdk_pkt__write_pcap( dumper2, pkts[i]);
						
						// if( fpkt)
						// {
							// dpdk_pkt__write_pcap( dumper2, fpkt);
						// }
			...
			case 56710:	//RTE_ETHER_TYPE_IPV6:
				{
					...
					{
						//DHCP Router Advitisement
						//refer packets in UPF Folder
					...
					else if( eth->dst_addr.addr_bytes[0] == interface->MacAddr[0] && eth->dst_addr.addr_bytes[1] == interface->MacAddr[1] && eth->dst_addr.addr_bytes[2] == interface->MacAddr[2] && eth->dst_addr.addr_bytes[3] == interface->MacAddr[3] && eth->dst_addr.addr_bytes[4] == interface->MacAddr[4] && eth->dst_addr.addr_bytes[5] == interface->MacAddr[5])
					{
						...
						*/
						...
						{
							ue_ip_hdr6 			= (struct rte_ipv6_hdr *)root_ip6_hdr;
							...
							ue_proto_packet = (char *)root_ip6_hdr + 40;
							iph_header_len = 40;
							...
							if( __dpe->SkipAPICalls == 0)
							{	
								if( cp__flow_allowed_ueipv6( ue_dst_ip6, ue_src_ip6, ue_dst_port, ue_src_port, ue_proto, p, pkts[i]->pkt_len, &sess, &p_ran_teid, PROCESS_TYPE__Gi_GTP, recv_worker_index, &qfi, &rgid, rci, &flowPtr) == 0)
								{
									if( __dpe->EnablePktLogs == 1)
									{
										dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "Gi Packet received from ue_ip=%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X  dst_ip=%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X  src_port=%u  dst_port=%u  protocol=%u teid=%u  EnableNAPT=%u  ProcessType=%u  IPv4=%u|%u sess=%p  and dropped", 
											ue_src_ip6[8] & 0xFF, ue_src_ip6[9] & 0xFF, ue_src_ip6[10] & 0xFF, ue_src_ip6[11] & 0xFF, ue_src_ip6[12] & 0xFF, ue_src_ip6[13] & 0xFF, ue_src_ip6[14] & 0xFF, ue_src_ip6[15] & 0xFF,
											ue_dst_ip6[8] & 0xFF, ue_dst_ip6[9] & 0xFF, ue_dst_ip6[10] & 0xFF, ue_dst_ip6[11] & 0xFF, ue_dst_ip6[12] & 0xFF, ue_dst_ip6[13] & 0xFF, ue_dst_ip6[14] & 0xFF, ue_dst_ip6[15] & 0xFF,
											ue_src_port, ue_dst_port, ue_proto, p_ran_teid, interface->EnableNAPT, interface->ProcessType, interface->IPv4, ue_dst_ip, sess);
									}
								...
								{
									dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "Gi Packet received from ue_ip=%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X  dst_ip=%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X  src_port=%u  dst_port=%u  protocol=%u  p_ran_teid=%u sess=%p  and allowed", 
											ue_src_ip6[8] & 0xFF, ue_src_ip6[9] & 0xFF, ue_src_ip6[10] & 0xFF, ue_src_ip6[11] & 0xFF, ue_src_ip6[12] & 0xFF, ue_src_ip6[13] & 0xFF, ue_src_ip6[14] & 0xFF, ue_src_ip6[15] & 0xFF,
											ue_dst_ip6[8] & 0xFF, ue_dst_ip6[9] & 0xFF, ue_dst_ip6[10] & 0xFF, ue_dst_ip6[11] & 0xFF, ue_dst_ip6[12] & 0xFF, ue_dst_ip6[13] & 0xFF, ue_dst_ip6[14] & 0xFF, ue_dst_ip6[15] & 0xFF,										
							...
							if( fpkt)
							{
								...
								// PRIMARY_PACKET_INFO
								// eth=14 + IPv4=20 + UPD=8 + GTP=16 = 
								__ue_ip6_hdr = (struct rte_ipv6_hdr *)&p[58];
							
								
								// printf( "toTrim=%u  bef-payload_len=%u aft=%u  prependLen=%u\n", 
									// toTrim, rte_cpu_to_be_16( __ue_ip6_hdr->payload_len), 
									// rte_cpu_to_be_16( rte_cpu_to_be_16( __ue_ip6_hdr->payload_len) - toTrim), prependLen);
								
								__ue_ip6_hdr->payload_len 		= rte_cpu_to_be_16( rte_cpu_to_be_16( __ue_ip6_hdr->payload_len) - toTrim);
								...
								if( ue_proto == IPPROTO_UDP)
								{
									...
									ue_udp->dgram_len 		= rte_cpu_to_be_16( rte_cpu_to_be_16( ue_udp->dgram_len) - toTrim);
									ue_udp->dgram_cksum		= 0;
									ue_udp->dgram_cksum		= rte_ipv6_udptcp_cksum( __ue_ip6_hdr, ue_udp);
								}
								else if( ue_proto == IPPROTO_TCP)
								{
									ue_tcp = (struct rte_tcp_hdr *)&p[58 + iph_header_len];
									ue_tcp->cksum = 0;
									ue_tcp->cksum = rte_ipv6_udptcp_cksum( __ue_ip6_hdr, ue_tcp);
								}
								// else if( ue_proto == 132)
								// {
								// }
							...
							*(uint16_t *)(((unsigned char *)gtp) + 8) = 0;	//seq no
							...
							// currently supporting only IPv4 for RAN
							if( ran_ipv == 4) 
							{
								udp->dgram_len 			= rte_cpu_to_be_16( (orig_pkt_len-14) + (prependLen-20));	// remove udp-len from prependLen
								udp->dgram_cksum		= rte_ipv4_udptcp_cksum( ip_hdr, udp);
							
								//printf( "dgram_len=%d dgram_cksum=%d  %d\n", udp->dgram_len, udp->dgram_cksum, __LINE__);
							} 
							else if( ran_ipv == 6) 
							{
								udp->dgram_len 			= rte_cpu_to_be_16( (orig_pkt_len-14) + gtpHeaderLen + 8);	// remove udp-len from prependLen
							...
							// printf( "ran-ipv=%u  prepend-len=%u  orig_pkt_len=%u new-len=%u  %d\n", ran_ipv, prependLen, orig_pkt_len, pkts[i]->pkt_len, __LINE__);
							...
							if( fpkt)
							{
								...
								// if( ue_proto == IPPROTO_UDP)
								// {
									// fue_udp  = (struct rte_udp_hdr *)&p[58 + iph_header_len];
									
									// fue_udp->dgram_len 			= rte_cpu_to_be_16( rte_cpu_to_be_16( ue_udp->dgram_len) - toTrim);
									// fue_udp->dgram_cksum		= 0;
									// fue_udp->dgram_cksum		= rte_ipv6_udptcp_cksum( __ue_ip6_hdr, fue_udp);
								// }
								// else if( ue_proto == IPPROTO_TCP)
								// {
									// fue_tcp = (struct rte_tcp_hdr *)&p[58 + iph_header_len];
									// fue_tcp->cksum = 0;
									// fue_tcp->cksum = rte_ipv6_udptcp_cksum( __ue_ip6_hdr, fue_tcp);
								// }
								
								
								
								froot_ip_hdr = (struct rte_ipv4_hdr *)&fp[14];
							
								froot_ip_hdr->total_length 	= rte_cpu_to_be_16( fpkt_len - 14);
								froot_ip_hdr->hdr_checksum 	= 0;
								froot_ip_hdr->hdr_checksum 	= rte_ipv4_cksum( froot_ip_hdr);
							
								
								froot_udp = (struct rte_udp_hdr *) ((unsigned char *)froot_ip_hdr + sizeof(struct rte_ipv4_hdr));
							
								gtp  					= (struct rte_gtp_hdr *) ((unsigned char *)froot_udp + sizeof(struct rte_udp_hdr));
								gtp->plen 				= rte_cpu_to_be_16( fpkt_len - (14 + 20 + 8));
							
								
								rte_memcpy( &fp[58], __ue_ip6_hdr, iph_header_len);
							
							
								fue_ip6_hdr = (struct rte_ipv6_hdr *)&fp[58];
								
								
								
								if( ue_proto == IPPROTO_UDP)
								{
									fue_ip6_hdr->payload_len 	= rte_cpu_to_be_16( toTrim + 8);									
									rte_memcpy( &fp[ 58 + iph_header_len], ue_udp, 8);
									...
									fue_udp->dgram_len 			= rte_cpu_to_be_16( fpkt_len - (58 + iph_header_len));
									fue_udp->dgram_cksum		= 0;
									fue_udp->dgram_cksum		= rte_ipv6_udptcp_cksum( fue_ip6_hdr, fue_udp);									
								}
								else if( ue_proto == IPPROTO_TCP)
								{
									fip_tcp_len = ( ue_tcp->data_off >> 4) * 4;
									
									fue_ip6_hdr->payload_len 	= rte_cpu_to_be_16( toTrim + fip_tcp_len);
									rte_memcpy( &fp[ 58 + iph_header_len], ue_tcp, fip_tcp_len);
									
									
									fue_tcp = (struct rte_tcp_hdr *)&fp[58 + iph_header_len];

									fue_tcp->sent_seq = rte_cpu_to_be_32( rte_cpu_to_be_32(ue_tcp->sent_seq) + ue_pl_len);

									fue_tcp->cksum = 0;
									fue_tcp->cksum = rte_ipv6_udptcp_cksum( fue_ip6_hdr, fue_tcp);
								}
								
								froot_udp->dgram_cksum 		= 0;
								froot_udp->dgram_len 		= rte_cpu_to_be_16( fpkt_len - (14 + 20));	// remove udp-len from prependLen
								froot_udp->dgram_cksum		= rte_ipv4_udptcp_cksum( froot_ip_hdr, froot_udp);
							}




							// pcap_dumper_t * dumper5 = dpdk_pkt__open_pcap_dump( "./upf_ipv6_gtp_20_11_2024_1.pcap");
							// dpdk_pkt__write_pcap( dumper5, pkts[j]);
							
							// if( fpkt)
							// {
								// dpdk_pkt__write_pcap( dumper5, fpkt);
							// }
...
int dpe__create_packet( struct rte_mbuf * m_buf, dpdk_interface_t * interface)
{
	uint32_t upf_ip 	= 0; 
	uint32_t upf_teid 	= 0; 
	uint32_t ue_ipv4 	= 0;
	
	if( cp__get_next_ueip( &upf_ip, &upf_teid, &ue_ipv4) == 0)
	{
		return 0;
	}
	...
	eth->src_addr.addr_bytes[0] = 0x00;
	eth->src_addr.addr_bytes[1] = 0x0C;
	eth->src_addr.addr_bytes[2] = 0x29;
	eth->src_addr.addr_bytes[3] = 0x04;
	eth->src_addr.addr_bytes[4] = 0x05;
	eth->src_addr.addr_bytes[5] = 0x06;
	
	memcpy( eth->dst_addr.addr_bytes, interface->MacAddr, 6);
	...
	ip_hdr->src_addr 		= upf_ip;
	ip_hdr->dst_addr 		= rte_cpu_to_be_32( upf_ip);
	ip_hdr->hdr_checksum 	= rte_ipv4_cksum( ip_hdr);
	...
	gtp->msg_type 				= 0xFF;
	gtp->plen 					= rte_cpu_to_be_16( prependLen - (14 + 20 + 8));
	gtp->teid 					= rte_be_to_cpu_32( upf_teid); 
	...
	usr_ip_hdr->src_addr 		= rte_cpu_to_be_32( ue_ipv4);
	usr_ip_hdr->dst_addr 		= ue_ipv4;
...
int dpe__receive( void * args)
{
	...
	dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "launching receive core for %s", interface_port->interface->Port);
	...
	dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "starting receive core for %s", interface_port->interface->Port);
	...
	if( __dpe->EnableSelfLoadTest == 1)
	{
		if( interface_port->interface->ProcessType == PROCESS_TYPE__Gi_GTP)
		{
			while(1) 
			{
				usleep( 999999);
				...
				if( cx > 5)
				{
					dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "EnableSelfLoadTest: This interface will not start in this mode");
					cx = 0;
				}
				cx++;
			}
		}
		else if( interface_port->interface->ProcessType == PROCESS_TYPE__GTP_Gi)
		{
			while(__dpe->StartSelfLoadTest == 0) 
			{
				usleep( 999999);
				
				if( cx > 5)
				{
					dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "EnableSelfLoadTest: waiting for StartSelfLoadTest=%u", __dpe->StartSelfLoadTest);
					cx = 0;
				}
				cx++;				
			}
	...
	// mbufs[0] = rte_pktmbuf_alloc( interface_port->interface->mempool[0]);
	
	// pcap_dumper_t * dumper2 = dpdk_pkt__open_pcap_dump( "./packet_test__27_03_2025.pcap");
	// dpe__create_packet( mbufs[0], interface_port->interface);
	// dpdk_pkt__write_pcap( dumper2, mbufs[0]);
	// dpdk_pkt__close_pcap( dumper2);
	// exit(0);	

	// if(__dpe->StartSelfLoadTest == 1) 
	// {
		// mbufs[0] = rte_pktmbuf_alloc( interface_port->interface->mempool[0]);
		// dpe__create_packet( mbufs[0], interface_port->interface);
		// exit(0);
	...
	//dpdk_interface_t * interface		= interface_port->interface;
	dpdk_interface_queue_t * if_queue 	= &interface_port->interface->if_queue[interface_port->queue_id];
	...
	while(__dpe->runing)
	{
		...
		if(__dpe->StartSelfLoadTest == 1) 
		{
			for( cx = 0; cx < interface_port->interface->RecvChunkSize; cx++)
			{
				mbufs[cx] = rte_pktmbuf_alloc( interface_port->interface->mempool[0]);
				...
				dpe__create_packet( mbufs[cx], interface_port->interface);
			}
			...
			// for( cx = 0; cx < interface_port->interface->RecvChunkSize; cx++)
			// {
				// dpdk_pkt__write_pcap( dumper2, mbufs[cx]);
			// }
		...
		{
			received_cnt = rte_eth_rx_burst( interface_port->interface->PortId, interface_port->interface->receive_queue[interface_port->queue_id], mbufs, interface_port->interface->RecvChunkSize);
		}
		...
		else 
		{
			...
			if_queue->total_received += received_cnt;
			pkts_send = 0;
					...
					if( (ip_hdr->next_proto_id == IPPROTO_ICMP || ip_hdr->next_proto_id == IPPROTO_UDP ||  ip_hdr->next_proto_id == IPPROTO_TCP) && ip_hdr->dst_addr == interface_port->interface->IPv4 )
					{
						if( ip_hdr->version_ihl == 0x45)
						{
							if( ip_hdr->next_proto_id == IPPROTO_UDP)
							{
								...
								nat_entry 	= dpe__nat__find_natentry_by_natsrcport( interface_port->interface, ip_hdr->dst_addr, udp->dst_port, ip_hdr->src_addr, udp->src_port, ip_hdr->next_proto_id);
							...
							else if(ip_hdr->next_proto_id == IPPROTO_TCP)
							{
								...
								nat_entry 	= dpe__nat__find_natentry_by_natsrcport( interface_port->interface, ip_hdr->dst_addr, tcp->dst_port, ip_hdr->src_addr, tcp->src_port, ip_hdr->next_proto_id);
							...
							else if(ip_hdr->next_proto_id == IPPROTO_ICMP)
							{
								...
								nat_entry 	= dpe__nat__find_natentry_by_natsrcport( interface_port->interface, ip_hdr->dst_addr, icmp_h->icmp_ident, ip_hdr->src_addr, icmp_h->icmp_seq_nb, ip_hdr->next_proto_id);
...
int dpe__recv_worker( void * args)
{
	...
	dpdk_interface_port_t * interface_port = (dpdk_interface_port_t *) args;
	dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "starting rec-worker core for %s", interface_port->interface->Port);

	
	struct rte_ring * ring = interface_port->interface->recv_ring[interface_port->queue_id];
	struct rte_mbuf * pkts[interface_port->interface->RecvChunkSize];
	
	struct rte_mbuf * processed_pkts[interface_port->interface->RecvChunkSize * 2];
	...
	dpdk_interface_queue_t * if_queue 	= &interface_port->interface->if_queue[ interface_port->worker_queue_id];
	...
	// if( __dpe->DPICoreCount == 0)
	// {
		// if( (interface_port->interface->CQosEnabled == 1) || (interface_port->interface->QosInitalized == 1 && interface_port->interface->QosConfigured == 1))
		// {
	...
	// recordered usage
	struct rte_mbuf * processed_pkts2[interface_port->interface->RecvChunkSize * 2];
	uint16_t processed_pkts_count2 = 0;
	uint16_t i2 = 0;
	
	struct rte_mbuf * processed_pkts3[interface_port->interface->RecvChunkSize * 2];
	uint16_t processed_pkts_count3 = 0;
	...
	while(__dpe->runing)
	{
		...
		else
		{
			...
			if_queue->total_dequeed += recv_pkts;
			...
			{
				//Forward
				processed_pkts_count = dpe__process_forward( interface_port->interface, pkts, recv_pkts, processed_pkts, drop_pkts, &drop_pkts_count, arp_pkts, &arp_pkts_count, gtp_pkts, &gtp_pkts_count, interface_port->recv_worker_index, interface_port->worker_queue_id, interface_port->rci);
			...
			if( drop_pkts_count > 0)
			{
				...
				if_queue->total_dropped += drop_pkts_count;
			...
			if( arp_pkts_count > 0)
			{
				...
				if_queue->arp_request_received += arp_pkts_count;
				if_queue->arp_response_sent += arp_pkts_count;
				//if_queue->total_enqueed += arp_pkts_count;
			}
			
			
			// GTP Control Messages, ECHO
			if( gtp_pkts_count > 0)
			{
				gtp_pkts_send = 0;
				do {
			...
			if( processed_pkts_count > 0)
			{
				...
				if( __dpe->DPICoreCount == 0)
				{
					if( (interface_port->interface->CQosEnabled == 1) || (interface_port->interface->QosInitalized == 1 && interface_port->interface->QosConfigured == 1))
					{
						...
						interface_port->interface->sched_enqueed += pkts_send;
					}
					else
					{
						processed_pkts_count2 = 0;
						drop_pkts_count = 0;
						
						for( i2 = 0; i2 < processed_pkts_count; i2++)
						{
							pdata = (privdata_t*)rte_mbuf_to_priv( processed_pkts[i2]);
							
							
							if( cp__record_usage( pdata->userdata1, interface_port->interface->ProcessType, pdata->rgid, processed_pkts[i2]->pkt_len, 0) == 1)
							{
								processed_pkts2[processed_pkts_count2] = processed_pkts[i2];
								processed_pkts_count2++;

								if( pdata->EnablePktLogs == 1 && __dpe->EnablePktLogs > 0)
								{
									dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "cp__record_usage: allowed packet for ue_ip=%u.%u.%u.%u  rgid=%d   %s|%s|%d", 
											pdata->ueip & 0xFF, (pdata->ueip >> 8) & 0xFF, (pdata->ueip >> 16) & 0xFF, (pdata->ueip >> 24) & 0xFF, 
										pdata->rgid, __FILE__, __FUNCTION__, __LINE__);
								}
							}
							else
							{
								drop_pkts[drop_pkts_count] = processed_pkts[i2];
								drop_pkts_count++;
								
								if( pdata->EnablePktLogs == 1 && __dpe->EnablePktLogs > 0)
								{
									dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "cp__record_usage: dropping packet for ue_ip=%u.%u.%u.%u   %s|%s|%d", 
											pdata->ueip & 0xFF, (pdata->ueip >> 8) & 0xFF, (pdata->ueip >> 16) & 0xFF, (pdata->ueip >> 24) & 0xFF, 
						...
						if( processed_pkts_count2 > 0)
						{
							do {
								pkts_send += rte_ring_enqueue_burst( send_ring, (void **)&processed_pkts2[pkts_send], processed_pkts_count2 - pkts_send, NULL);
							} while( pkts_send < processed_pkts_count2);
							
							if_queue->total_enqueed += pkts_send;
						}
					}
				}
				else
				{
					processed_pkts_count2 = 0;
					processed_pkts_count3 = 0;
					
					for( pkts_send = 0; pkts_send < processed_pkts_count; pkts_send++)
					{
						pdata = (privdata_t*)rte_mbuf_to_priv( processed_pkts[pkts_send]);

						if( pdata->rgid == 0) {
							
							if( pdata->flowIndex == 0)
							{
								pdata->flowIndex = 1;

								if( __dpe->DPICoreCount > 1)
								{
									pdata->flowIndex = (pkts_send % __dpe->DPICoreCount) + 1;
								}
								
								cp__set_flowIndex( pdata->flowptr, pdata->flowIndex);
							}
							
							// we cannot push bulk here, ite based on flow-index, we need to send to respective queue
							rte_ring_enqueue( __dpe->dpi_ring[pdata->flowIndex - 1], processed_pkts[pkts_send]);
						} 
						else 
						{
							processed_pkts3[processed_pkts_count3] = processed_pkts[pkts_send];
							processed_pkts_count3++;
							//rte_ring_enqueue( send_ring, processed_pkts[pkts_send]);
						}
					}
					
					if( processed_pkts_count3 > 0)
					{
						processed_pkts_count2 = 0;
						drop_pkts_count = 0;
						
						for( i2 = 0; i2 < processed_pkts_count3; i2++)
						{
							pdata = (privdata_t*)rte_mbuf_to_priv( processed_pkts3[i2]);
							
							if( cp__record_usage( pdata->userdata1, interface_port->interface->ProcessType, pdata->rgid, processed_pkts3[i2]->pkt_len, 0) == 1)
							{
								processed_pkts2[processed_pkts_count2] = processed_pkts3[i2];
								processed_pkts_count2++;
							}
							else
							{
...
int dpe__generator_worker( void * args)
{
	...
	if( ptrPacketGeneratorFunction > 0) 
	{
		...
		struct rte_mempool * mpool = rte_pktmbuf_pool_create( "mpool1", 131072, 512, rte_align32pow2(sizeof(privdata_t)), RTE_MBUF_DEFAULT_BUF_SIZE, rte_socket_id());
...
int dpe__send( void * args)
{
	...
	dpdk_interface_port_t * interface_port = (dpdk_interface_port_t *) args;
	dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "dpe__send  starting send core for %s", interface_port->interface->Port);
	...
	while(__dpe->runing) 
	{
		...
		else
		{
				...
				for( cx = 0; cx < recv_pkts; cx++)
				{
					...
					if( (ip_hdr->next_proto_id == IPPROTO_ICMP || ip_hdr->next_proto_id == IPPROTO_UDP || ip_hdr->next_proto_id == IPPROTO_TCP) && ip_hdr->src_addr != interface_port->interface->IPv4 )
					{
						if( ip_hdr->version_ihl == 0x45)
						{
							...
							{
								udp	= (struct rte_udp_hdr *)&pkt[ 14 + 20 ];
								nat_entry = dpe__nat__find_or_create_natentry( interface_port->interface, ip_hdr->src_addr, ip_hdr->dst_addr, udp->src_port, udp->dst_port, ip_hdr->next_proto_id);
							...
							{
								tcp	= (struct rte_tcp_hdr *)&pkt[ 14 + 20 ];
								nat_entry = dpe__nat__find_or_create_natentry( interface_port->interface, ip_hdr->src_addr, ip_hdr->dst_addr, tcp->src_port, tcp->dst_port, ip_hdr->next_proto_id);
							...
							else if(ip_hdr->next_proto_id == IPPROTO_ICMP)
							{
								...
								nat_entry = dpe__nat__find_or_create_natentry( interface_port->interface, ip_hdr->src_addr, ip_hdr->dst_addr, icmp_h->icmp_ident, icmp_h->icmp_seq_nb, ip_hdr->next_proto_id);
			...
			if( __dpe->pdumper)
			{
				for( cx = 0; cx < recv_pkts; cx++)
				{
					rte_prefetch0( rte_pktmbuf_mtod( pkts[cx], void *));
			...
			if( __dpe->EnablePktLogs == 1)
			{
				for( cx = 0; cx < recv_pkts; cx++)
				{
					pdata = (privdata_t*)rte_mbuf_to_priv( pkts[cx]);
					
					if( pdata->ueip > 0)
					{
						if( interface_port->interface->ProcessType == PROCESS_TYPE__Gi_GTP)
						{
							dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, 
								"Sending Gi to ISP: ue_ip=%u.%u.%u.%u", pdata->ueip & 0xFF, (pdata->ueip >> 8) & 0xFF, (pdata->ueip >> 16) & 0xFF, (pdata->ueip >> 24) & 0xFF);
						}
						else if( interface_port->interface->ProcessType == PROCESS_TYPE__GTP_Gi)
						{
							dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, 
								"Sending GTP to RAN: ue_ip=%u.%u.%u.%u", pdata->ueip & 0xFF, (pdata->ueip >> 8) & 0xFF, (pdata->ueip >> 16) & 0xFF, (pdata->ueip >> 24) & 0xFF);							
						}
					}
				}
			}
			...
			do 
			{
				...
				sent = rte_eth_tx_burst( interface_port->interface->PortId , interface_port->interface->send_queue[ interface_port->queue_id], &pkts[pkts_send], recv_pkts - pkts_send);
...
int dpe__cqos( void * interafaceArgs);
...
void dpe__configure_interface( dpdk_interface_t * interaface)
{
	...
	if (0 != ret)
	{
		...
		dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "Error: rte_eth_dev_get_port_by_name failed for BusId=%s\n", interaface->Port);
		exit(0);
	}
	...
	if (1 != ret)
	{
		...
		dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "Error: rte_eth_dev_is_valid_port failed for BusId=%s\n", interaface->Port);
		exit(0);
	}
	...
	else 
	{
		if( mtu > 1500) 
		{
			...
			if( ret != 0) 
			{
				...
				dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "Error: mtu size set failed for BusId=%s\n", interaface->Port);
				exit(0);
			}
		}
	}
	...
	if (0 != ret) 
	{
		...
		dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "Error: rte_eth_dev_info_get failed BusId=%s\n", interaface->Port);
		exit(0);
	}
	...
	if( interaface->RecvQueue > dev_info.max_rx_queues)
	{
		...
		dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "Error: BusId:%s, configured RQ=%d is > Supported RQ=%d\n", interaface->Port, interaface->RecvQueue, dev_info.max_rx_queues);
		exit(0);
	}
	...
	if( interaface->SendQueue > dev_info.max_tx_queues)
	{
		...
		dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "BusId:%s, configured SQ=%d is > Supported SQ=%d", interaface->Port, interaface->SendQueue, dev_info.max_tx_queues);
		exit(0);
	}
	
	dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "Total Queues   Receive=%u  Send=%u", interaface->RecvQueue, interaface->SendQueue);
	...
	if(!interaface->mempool)
	{
		...
		dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "Error: memory allocation failed for port: BusId=%s", interaface->Port);
		exit(0);
	}
	...
	for( mi = 0; mi < interaface->RecvQueue; mi++)
	{
		...
		interaface->mempool[mi] = rte_pktmbuf_pool_create( mempool_name, interaface->MemPoolSize, interaface->MemPoolCacheSize, rte_align32pow2(sizeof(privdata_t)), RTE_MBUF_DEFAULT_BUF_SIZE, rte_socket_id());
		...
		if(!interaface->mempool[mi])
		{
			...
			dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "Error: memory allocation failed for port: BusId=%s Index=%u  mi=%u", interaface->Port, interaface->Index, mi);
			exit(0);
		}
		
		dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "Created Mempool  BusId=%s for Receive Queue=%d", interaface->Port, mi);
	}
	...
	if (ret != 0)
	{
		...
		dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "rte_eth_dev_configure(%s(%d), rx=%u, tx=%u ) failed   %s|%s|%d", 
			interaface->Port, interaface->PortId, interaface->RecvQueue, interaface->SendQueue, __FILE__, __FUNCTION__, __LINE__);	
	...
	if( interaface->RxDescLimit > 0)
	{
		rx_sz = interaface->RxDescLimit;
	}

	if( interaface->TxDescLimit > 0)
	{
		tx_sz = interaface->TxDescLimit;
	}
	...
	if (ret != 0)
	{
		...
		dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "rte_eth_dev_adjust_nb_rx_tx_desc(%s(%d), rx=%u, tx=%u ) failed     %s|%s|%d", interaface->Port, interaface->PortId, rx_sz, tx_sz, __FILE__, __FUNCTION__, __LINE__);
	...
	for( i = 0; i < interaface->RecvQueue; i++ ) 
	{
		...
		ret = rte_eth_rx_queue_setup( interaface->PortId, interaface->receive_queue[i], rx_sz, rte_eth_dev_socket_id( interaface->PortId), NULL, interaface->mempool[i]);
		...
		if (0 != ret)
		{
			...
			dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "rte_eth_rx_queue_setup failed ifid=%s port_id=%u rx_q_count=%u  %s|%s|%d", interaface->Port, interaface->PortId, interaface->RecvQueue, __FILE__, __FUNCTION__, __LINE__);
			exit(0);
		}
		
		dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "rx_queue_setup completed  BusId=%s for Receive Queue=%d", interaface->Port, i);
	}
	...
	for( i = 0; i < interaface->SendQueue; i++ )
	{
		...
		ret = rte_eth_tx_queue_setup( interaface->PortId, interaface->send_queue[i], tx_sz, rte_eth_dev_socket_id( interaface->PortId), &tx_conf);
		...
		if (0 != ret)
		{
			...
			dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "rte_eth_tx_queue_setup failed ifid=%s port_id=%u tx_q_count=%u  %s|%s|%d", interaface->Port, interaface->PortId, interaface->SendQueue, __FILE__, __FUNCTION__, __LINE__);
			exit(0);
		}
		
		dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "tx_queue_setup completed  BusId=%s for Receive Queue=%d", interaface->Port, i);
	}
	...
	for( i = 0; i < interaface->RecvQueue; i++ )
	{
		...
		if(!interaface->recv_ring[i])
		{
			...
			dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "rte_ring_create failed for recv BusId=%s at %d", interaface->Port, i);
			exit(0);
		}
		
		dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "Created Receive Ring  BusId=%s for  Receive Queue=%d", interaface->Port, i);
	}


	//if( interaface->QosInitalized == 1)
	if( interaface->QosEnabled == 1 || interaface->CQosEnabled == 1)
	{
		memset( recv_ring_name, 0, sizeof(recv_ring_name));
		...
		dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "Created Ring for Schduler  BusId=%s", interaface->Port);
	}
	...
	for( i = 0; i < interaface->SendQueue; i++ )
	{
		...
		dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "Created Send Ring for BusId=%s Send Queue=%d", interaface->Port, i);
	}
	...
	for( i = 0; i < (interaface->RecvWorkerCount * interaface->RecvWorkerCountPerQ); i++ )
	{
		...
		for( j = 0; j < interaface->RecvWorkerCountPerQ; j++ )
		{
			...
			interaface_port->worker_queue_id 	= z;
			interaface_port->rci 				= z;
			interaface_port->port_type 			= 1;
	...
	interaface->TotalWorkers = z;
	dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "Total Workers for Port=%s  TotalWorkers=%u", interaface->Port, interaface->TotalWorkers);	

	usleep( 111111);


	if( interaface->CQosEnabled == 1)
	{
		rte_eal_remote_launch( dpe__cqos,  	interaface, __dpe->last_lcore++);
	}
	else if( interaface->QosEnabled == 1)
	{
...
void * dpe__run_pref_thread( void * args)
{
	...
	while( __dpe->runing)
	{
		...
		while( interaface)
		{
			if( interaface->DestIP > 0 && ((interaface->DstMacReceived == 0 && (interaface->DstMacTime + 5) < ltime ) || (interaface->DstMacReceived == 1 && (interaface->DstMacTime + 30) < ltime )) )
			{
				struct rte_mbuf * m_buf = rte_pktmbuf_alloc( interaface->mempool[0]);
				...
				interaface->DstMacTime = (uint64_t)time(NULL);
			}
			...
			*/
			...
			for( i = 0; i < interaface->RecvQueue; i++)
			{
				received_c 	= interaface->if_queue[i].total_received - interaface->if_queue[i].last_received;
				dpe__perf_log( "%-40s  Queue %-4u  : %8lu %10lu\n", "Packets Received", i, received_c, interaface->if_queue[i].total_received );
				interaface->if_queue[i].last_received 	= interaface->if_queue[i].total_received;
			}

			for( i = 0; i < interaface->TotalWorkers; i++)
			{
				received_c 	= interaface->if_queue[i].total_dequeed - interaface->if_queue[i].last_dequeed;
				dpe__perf_log( "%-40s  Index %-4u  : %8lu %10lu\n", "Worker Dequeued Pkts", i, received_c, interaface->if_queue[i].last_dequeed);
				interaface->if_queue[i].last_dequeed = interaface->if_queue[i].total_dequeed;
				
				
				if( interaface->CQosEnabled == 0 && interaface->QosEnabled == 0)
				{
					received_c 	= interaface->if_queue[i].total_enqueed - interaface->if_queue[i].last_enqueed;
					dpe__perf_log( "%-40s  Index %-4u  : %8lu %10lu\n", "Worker Enqueed Pkts", i, dequeed_c, interaface->if_queue[i].total_enqueed);
					interaface->if_queue[i].last_enqueed = interaface->if_queue[i].total_enqueed;
				}
			}

			if( interaface->CQosEnabled == 1 || interaface->QosEnabled == 1)
			{
				received_c 	= interaface->sched_enqueed - interaface->last_sched_enqueed;
				dpe__perf_log( "%-40s              : %8lu %10lu\n", "Enqueed to QoS", received_c, interaface->sched_enqueed);
				interaface->last_sched_enqueed = interaface->sched_enqueed;
				
				received_c 	= interaface->sched_dequeed - interaface->last_sched_dequeed;
				dpe__perf_log( "%-40s              : %8lu %10lu\n", "QoS Dequeed", received_c, interaface->sched_dequeed);
				interaface->last_sched_dequeed = interaface->sched_dequeed;

				received_c 	= interaface->sched_dropped - interaface->last_sched_dropped;
				dpe__perf_log( "%-40s              : %8lu %10lu\n", "QoS Dropped", received_c, interaface->sched_dropped);
				interaface->last_sched_dropped = interaface->sched_dropped;

				received_c 	= interaface->sched_forwarded - interaface->last_sched_forwarded;
				dpe__perf_log( "%-40s              : %8lu %10lu\n",  "QoS Forwarded", received_c, interaface->sched_forwarded);
				interaface->last_sched_forwarded = interaface->sched_forwarded;
			}

			for( i = 0; i < interaface->SendQueue; i++)
			{
				received_c 	= interaface->if_queue[i].total_sent - interaface->if_queue[i].last_sent;
				dpe__perf_log( "%-40s  Queue %-4u  : %8lu %10lu\n", "Packets Sent", i, received_c, interaface->if_queue[i].total_sent );
				interaface->if_queue[i].last_sent = interaface->if_queue[i].total_sent;
			}

			dpe__perf_log( "%-40s        %-8lu\n",  "ARP Request Sent", interaface->arp_request_sent);
			dpe__perf_log( "%-40s        %-8lu\n",  "ARP Response Received", interaface->arp_response_received);
			dpe__perf_log( "%-40s        %-8lu\n",  "ARP Request Received", interaface->arp_request_received);
			dpe__perf_log( "%-40s        %-8lu\n",  "ARP Response Sent", interaface->arp_response_sent);
			...
			//if( x == 10)
			{
				...
				if( rte_eth_stats_get( interaface->PortId, &stats) == 0)
				{
					...
					//for( zx = 0; zx <  interaface->RecvQueue; zx++)
					{
						...
						dpe__perf_log( "NIC Port Stats: %s  ipackets: %lu - %lu     opackets: %lu - %lu     ierrors: %lu - %lu     oerrors: %lu - %lu     rx_nombuf: %lu - %lu\n", 
							interaface->Port, 
		...
		if( __dpe->IPv4HT)
		{
			dpe__perf_log( "%-40s        %-8u  %-8u\n",  "IPv4 Flow Hash Table", rte_hash_count(__dpe->IPv4HT), __dpe->IPv4HTCount);
		}
		
		if( __dpe->IPv6HT)
		{
			dpe__perf_log( "%-40s        %-8u  %-8u\n",  "IPv6 Flow Hash Table", rte_hash_count(__dpe->IPv6HT), __dpe->IPv6HTCount);
		}
...
int dpe__dpi( void * args)
{
	dpe_dpi_ins_t * dpi_ins = (dpe_dpi_ins_t *)args;
	uint32_t index = dpi_ins->index;
	
	struct rte_ring * send_ring = NULL;
	struct rte_ring * dpi_ring = __dpe->dpi_ring[index];
	struct rte_mbuf * mbuf = NULL;
	privdata_t * pdata = NULL;
	dpdk_interface_t * interface = NULL;
	dpdk_interface_t * txInterface = NULL;
	...
	struct rte_mbuf * fwd_pkts[64];
	struct rte_mbuf * drop_pkts[64];
	...
	while(1)
	{
		recv_pkts = rte_ring_dequeue_burst( dpi_ring, (void **)pkts, 64, NULL);
		...
		if( recv_pkts > 0)
		{
			...
			for( zx = 0; zx < recv_pkts; zx++)
			{
				mbuf = pkts[zx];
				rte_prefetch0( rte_pktmbuf_mtod( mbuf, void *));

				pdata = (privdata_t*)rte_mbuf_to_priv( mbuf);
			
				if( pdata)
				{
					interface 	= pdata->srcInterface;
					txInterface = pdata->dstInterface;

					switch( interface->ProcessType)
					{
						case PROCESS_TYPE__GTP_Gi:
							{
								pdata->rgid = cp__packet_dectection( index, pdata->iph, pdata->iphlen, 0);
							}
							break;
						case PROCESS_TYPE__Gi_GTP:
							{
								pdata->rgid = cp__packet_dectection( index, pdata->iph, pdata->iphlen, 1);
							}
							break;
						default:
							break;
					...
					if( pdata->EnablePktLogs == 1 && pdata->ueip > 0)
					{
						dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "DPI: moving packet to next stage   - ue_ip=%u.%u.%u.%u", pdata->ueip & 0xFF, (pdata->ueip >> 8) & 0xFF, (pdata->ueip >> 16) & 0xFF, (pdata->ueip >> 24) & 0xFF);
					}
					
					enq_sts = 0;
					
					if( ( interface->CQosEnabled == 1) || (interface->QosInitalized == 1 && interface->QosConfigured == 1))
					{
						for( max_i = 0; max_i < max_tries; max_i++)
						{
							if( rte_ring_enqueue( interface->sched_ring, mbuf) == 0)
							{
								enq_sts = 0;
								break;
							}
					...
					{
						if( cp__record_usage( pdata->userdata1, interface->ProcessType, pdata->rgid, mbuf->pkt_len, 0) == 1)
						{
							if( txInterface->SendQueue == 1)
							{
								rr_index = 0;
						...
						else
						{
							...
							if( pdata->EnablePktLogs == 1 && pdata->ueip > 0)
							{
								dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "DPI: quota not allowed   - ue_ip=%u.%u.%u.%u", pdata->ueip & 0xFF, (pdata->ueip >> 8) & 0xFF, (pdata->ueip >> 16) & 0xFF, (pdata->ueip >> 24) & 0xFF);
							}
						}
					}
					...
					if( enq_sts == 1)
					{
						if( pdata->EnablePktLogs == 1 && pdata->ueip > 0)
						{
							dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "DPI: dropping packet enq-failed   - ue_ip=%u.%u.%u.%u", pdata->ueip & 0xFF, (pdata->ueip >> 8) & 0xFF, (pdata->ueip >> 16) & 0xFF, (pdata->ueip >> 24) & 0xFF);
						}
						
						drop_pkts[drop_count] = mbuf;
						drop_count++;
					}
				} 
				else 
				...
				if( drop_count > 0)
				{
					rte_pktmbuf_free_bulk( drop_pkts, drop_count);
				}
			}
		} 
		else 
...
int dpe__cqos( void * interafaceArgs)
{
	...
	dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "launching sched core for Port=%s  side=%d", interface->Port, side);
	...
	struct rte_mbuf * pmbufs[ 64];
	struct rte_mbuf * fmbufs[ 64];
	int temp = 0; 

	int rr_index = 0;
	int iterator = 0;
	
	while(!interface->TxInterface)
	{
		usleep( 555555);
	}
	...
	if(!txInterface)
	{
		printf("TxInterface is null in sched, interface index=%u|%u\n", interface->Index, __LINE__);
		exit(0);
	}
	...
	while( __dpe->runing)
	{
		...
		interface->sched_dequeed += nb_pkt;

		
		for( ix = 0; ix < nb_pkt; ix++)
		{
			rte_prefetch0( rte_pktmbuf_mtod( mbufs[ix], void *));
			pdata = (privdata_t*)rte_mbuf_to_priv( mbufs[ix]);
			
			if( pdata)
			{
				if( cp__record_usage( pdata->userdata1, interface->ProcessType, pdata->rgid, mbufs[ix]->pkt_len, 1) == 1)
				{
					pmbufs[nb_pkt2] = mbufs[ix];
					nb_pkt2++;
					
					if( pdata->EnablePktLogs == 1 && pdata->ueip > 0)
					{
						dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "CQoS: moving packet to next stage   - ue_ip=%u.%u.%u.%u", pdata->ueip & 0xFF, (pdata->ueip >> 8) & 0xFF, (pdata->ueip >> 16) & 0xFF, (pdata->ueip >> 24) & 0xFF);
					}
				}
				else
				{
					fmbufs[nb_pkt3] = mbufs[ix];
					nb_pkt3++;

					if( pdata->EnablePktLogs == 1 && pdata->ueip > 0)
					{
						dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "CQoS: dropping packet   - ue_ip=%u.%u.%u.%u", pdata->ueip & 0xFF, (pdata->ueip >> 8) & 0xFF, (pdata->ueip >> 16) & 0xFF, (pdata->ueip >> 24) & 0xFF);
					}
				}
			}
			else
		...
		if (likely(nb_pkt3 > 0))
		{
			interface->sched_dropped += nb_pkt3;
			rte_pktmbuf_free_bulk( fmbufs, nb_pkt3);
		}
		...
		if (likely(nb_pkt2 > 0))
		{
			...
			do {
				pkts_send += rte_ring_enqueue_burst( send_ring, (void **)&pmbufs[pkts_send], nb_pkt2 - pkts_send, NULL);
			} 
			while( pkts_send < nb_pkt2);
			
			interface->sched_forwarded += nb_pkt2;
		}
...
int dpe__sched_init( void * interafaceArgs)
{
	...
	if(!txInterface)
	{
		printf("TxInterface is null in sched, interface index=%u|%d\n", interaface->Index, __LINE__);
		exit(0);
	}
...
{
	printf("\n");
	printf("setting up dpdk enviroment\n");
	...
	if( __dpdk_env__create_huge_pages() == 1)
	{
		__dpdk_env__mount();
	}
...
uint8_t * dpe_find_ipv4flow( uint32_t ueip, uint32_t dstip, uint16_t ueport, uint16_t dstport, uint8_t protocol)
{
	...
	uint8_t * flowInfo = NULL;
	rte_hash_lookup_data( __dpe->IPv4HT, (const void *)&flow, (void **)&flowInfo);	
	
	if( __dpe->EnablePktLogs > 0) 
	{
		uint32_t enabledForUEIP = dpe__pkt_log_enabled( ueip);
		
		if( enabledForUEIP == 1) 
		{
			dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "find_ipv4flow  - ue_ip=%u.%u.%u.%u   ueip=%u ueport=%u dstip=%u dstport=%u protocol=%u  flowInfo=%p", 
				(ueip >> 24) & 0xFF, (ueip >> 16) & 0xFF, (ueip >> 8) & 0xFF, ueip & 0xFF, 
				ueip, ueport, dstip, dstport, protocol, flowInfo);
		}
	}
...
uint8_t * dpe_find_ipv6flow( uint8_t * ueip, uint8_t * dstip, uint16_t ueport, uint16_t dstport, uint8_t protocol)
{
	...
	uint8_t * flowInfo = NULL;
	rte_hash_lookup_data( __dpe->IPv6HT, (const void *)&flow, (void **)&flowInfo);	
	return flowInfo;
}
...
int dpe_add_ipv4flow( uint32_t ueip, uint32_t dstip, uint16_t ueport, uint16_t dstport, uint8_t protocol, void * ip4flow)
{
	...
	int sts = rte_hash_add_key_data( __dpe->IPv4HT, (const void*)&flow, ip4flow);	
	
	if( sts < 0)
	{
		dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "ADD-FAILED add_ipv4flow  - ue_ip=%u.%u.%u.%u  ueip=%u ueport=%u dstip=%u dstport=%u protocol=%u  ip4flow=%p  sts=%u", 
				(ueip >> 24) & 0xFF, (ueip >> 16) & 0xFF, (ueip >> 8) & 0xFF,ueip & 0xFF,  
				ueip, ueport, dstip, dstport, protocol, ip4flow, sts);	
	}
	...
	if( __dpe->EnablePktLogs > 0) 
	{
		...
		if( enabledForUEIP == 1) 
		{
			dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "add_ipv4flow  - ue_ip=%u.%u.%u.%u  ueip=%u ueport=%u dstip=%u dstport=%u protocol=%u  ip4flow=%p", 
				(ueip >> 24) & 0xFF, (ueip >> 16) & 0xFF, (ueip >> 8) & 0xFF,ueip & 0xFF, 
				ueip, ueport, dstip, dstport, protocol, ip4flow);
		}
	}


	//uint8_t * flowInfoOut = NULL;
	//rte_hash_lookup_data( __dpe->IPv4HT, (const void *)&flow, (void **)&flowInfoOut);	 
	//printf("2 ueip=%u dstip=%u dstport=%u ueport=%u protocol=%u  ip4flow=%p  sts=%d  %d\n",ueip,dstip, dstport ,ueport, protocol, flowInfoOut, sts, __LINE__);
	//dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__,"ueip=%u dstip=%u dstport=%u ueport=%u protocol=%u  ip4flow=%p  sts=%d",ueip,dstip, dstport ,ueport, protocol, flowInfoOut, sts);
}
...
int dpe_add_ipv6flow( uint8_t * ueip, uint8_t * dstip, uint16_t ueport, uint16_t dstport, uint8_t protocol, void * ip4flow)
{
	...
	return rte_hash_add_key_data( __dpe->IPv6HT, (const void*)&flow, ip4flow);	
}
...
int dpe_del_ipv4flow( uint32_t ueip, uint32_t dstip, uint16_t ueport, uint16_t dstport, uint8_t protocol)
{
	...
	return rte_hash_del_key( __dpe->IPv4HT, (const void*)&flow); 	
}
...
int dpe_del_ipv6flow( uint8_t * ueip, uint8_t * dstip, uint16_t ueport, uint16_t dstport, uint8_t protocol)
{
	...
	return rte_hash_del_key( __dpe->IPv6HT, (const void*)&flow);	
}

void pfcp_stack___delipflows();

int dpe__ipflows_worker( void * args)
{
	while(1)
	{
		pfcp_stack___delipflows();
		usleep(999999);
	}
	return 0;
}
...
void dpe__init_engine( int argc, char **argv)
{
	...
	if( __dpe->ExecEnvSetup == 1)
	{
		dpe__setup_env(); 
	}
	
	printf("UPF-EAL:  calling rte_eal_init \n");
	
	int ret = rte_eal_init( argc, argv);
	if (ret < 0)
	{
		rte_panic("UPF-EAL:    Cannot init EAL\n");
		exit(0);
	}
	printf("UPF-EAL:  rte_eal_init - successful\n\n");	
	...
	{
		printf("UPF-EAL:  Error: Required Cores:%d, Available cores:%d\n", totalCoresRequired, rte_lcore_count());
		exit(0);
	}
	
	printf("UPF-EAL:  Required Cores/vCPUs=%u are available, total vCPUs=%u\n", totalCoresRequired, rte_lcore_count());
	...
	if( __dpe->ExecEnvSetup == 1)
	{
		int driver_attached_status = 0;
		int configured_now = 0;
		
		if( strlen( __dpe->dpdkToolsPath) > 0)
		{
			while( interaface)
			{
				if( strlen( interaface->ifName) > 0 && strlen( interaface->Port) > 0)
				{
					driver_attached_status = __dpdk_env__is_dpdk_driver_attached( interaface->Port);
					
					if( driver_attached_status == 0)
					{
						printf("UPF-EAL:  eth=%s attaching to dpdk-driver |%d\n", interaface->ifName, __LINE__);
						
						__dpdk_env__configure__pci( __dpe->dpdkToolsPath, interaface->ifName, interaface->Port);
						configured_now++;
					}
					else if( driver_attached_status == 1)
					{
						printf("UPF-EAL:  eth=%s already attached to dpdk-driver |%d\n", interaface->ifName, __LINE__);
					}
				}
				interaface = interaface->Next;
			}
	...
	if( __dpe->IPv4HTCount > 0)
	{
		struct rte_hash_parameters params;
		memset( &params, 0, sizeof(struct rte_hash_parameters));

		params.name					= "IPv4HT";
		params.entries 				= __dpe->IPv4HTCount;
		params.key_len 				= sizeof(dpdk_ipv4_key_t);
		params.hash_func 			= rte_jhash;
		params.hash_func_init_val 	= 0;
		params.socket_id 			= rte_socket_id();
		params.extra_flag			= RTE_HASH_EXTRA_FLAGS_RW_CONCURRENCY;
		
		__dpe->IPv4HT				= rte_hash_create( &params);
		
		if(!__dpe->IPv4HT)
		{
			printf( "IPv4HT creation failed\n");
			exit(0);
		}
	}
	
	if( __dpe->IPv6HTCount > 0)
	{
		struct rte_hash_parameters params;
		memset( &params, 0, sizeof(struct rte_hash_parameters));

		params.name					= "IPv6HT";
		params.entries 				= __dpe->IPv6HTCount;
		params.key_len 				= sizeof(dpdk_ipv6_key_t);
		params.hash_func 			= rte_jhash;
		params.hash_func_init_val 	= 0;
		params.socket_id 			= rte_socket_id();
		params.extra_flag			= RTE_HASH_EXTRA_FLAGS_RW_CONCURRENCY;
		
		__dpe->IPv6HT				= rte_hash_create( &params);
		
		if(!__dpe->IPv6HT)
		{
			printf( "IPv6HT creation failed\n");
			exit(0);
		}		
	}	

	__dpe->runing = 1;
	__dpe->last_lcore = rte_get_next_lcore( 0, 1, 0) + 1;
	...
	rte_eal_remote_launch( dpe__ipflows_worker,  NULL, __dpe->last_lcore++);
	...
	while( interaface)
	{
		dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "Configuring Interface  BusId=%s  Port=%d", interaface->Port, interaface->PortId);
		...
		dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "Configuring Interface Completed BusId=%s  Port=%d", interaface->Port, interaface->PortId);
		dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "----------------------------------------------------------------------------------------------------------");
		interaface = interaface->Next;
	}
	...
	if( __dpe->DPICoreCount > 0)
	{
		// int 			DPICoreCount;
		// dpe_dpi_ins_t 	dpe_dpi_ins[40];
		// struct rte_mempool * dpe_mempool;
		// struct rte_ring    * dpe_recv[40];		

		int dcc = 0;
		char dpi_ring_name[30];
		
		for( dcc = 0; dcc < __dpe->DPICoreCount; dcc++)
		{
			memset( dpi_ring_name, 0, sizeof(dpi_ring_name));
			sprintf( dpi_ring_name, "dpi-ring-%d", dcc);
			
			__dpe->dpi_ring[dcc] = rte_ring_create( dpi_ring_name, rte_align32pow2(4096), rte_socket_id(), RING_F_SP_ENQ | RING_F_SC_DEQ);
		}
		
		for( dcc = 0; dcc < __dpe->DPICoreCount; dcc++)
		{
			__dpe->dpe_dpi_ins[dcc].index = dcc;
			rte_eal_remote_launch( dpe__dpi,  &__dpe->dpe_dpi_ins[dcc], __dpe->last_lcore++);
		}
	}
	...
	dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "Logger Started, EnablePktLogs=%d", __dpe->EnablePktLogs);
...
void dpe__test__encode_icmppacket_to_ran()
{
	...
	uint16_t cnt = dpe__process_forward( interface, pkts, recv_pkts, processed_pkts, dropped_mbufs, &dropped_cnt_out, arp_mbufs, &arp_cnt_out, gtp_mbufs, &gtp_cnt_out, 0, 0, 0);
...
void dpe__test__create_single_session()
{
	...
	int res1 = cp__flow_allowed_sess_v4( session, 0x0A2D0001 + 100000, 5500, 6600, 6, dataPkt, pkt_len, PROCESS_TYPE__GTP_Gi, 0, &rgid, 0, &flowPtr);
	printf("cp__flow_allowed_sess_v4 res1=%u session=%p  %s|%d\n", res1, session, __FUNCTION__, __LINE__);

	
	session = NULL;
	int res2 = cp__flow_allowed_ueipv4( htonl(0x0A2D0001), 0x0A2D0001 + 100000, 6600, 5500, 6, dataPkt, pkt_len, &session, &p_ran_teid, PROCESS_TYPE__Gi_GTP, 0, &qfi, &rgid, 0, &flowPtr);
...
void dpe__test__create_single_packet( int protocol)
{
	...
	if( cnt == 1)
	{
		...
		cnt = dpe__process_forward( interface, pkts, recv_pkts, processed_pkts, dropped_mbufs, &dropped_cnt_out, arp_mbufs, &arp_cnt_out, gtp_mbufs, &gtp_cnt_out, 0, 0, 0);
		printf("cnt=%d  dropped_cnt_out=%d  arp_cnt_out=%d  %s|%d\n", cnt, dropped_cnt_out, arp_cnt_out, __FUNCTION__, __LINE__);
...
void dpe__test__run_gtp_gi_load()
{
	...
	while(1) 
	{
		...
		dpe__process_forward( interface, pkts, recv_pkts, processed_pkts, dropped_mbufs, &dropped_cnt_out, arp_mbufs, &arp_cnt_out, gtp_mbufs, &gtp_cnt_out, 0, 0, 0);
