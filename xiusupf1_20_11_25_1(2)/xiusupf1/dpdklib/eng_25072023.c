...
int dpe__log( unsigned long mLoggingLevel, const char *fileName, int lineNum, char* mLogMessage, ...)
{
	...
	write( __dpe->iLogHandle, f_buffer, strlen( f_buffer));
...
int dpe__perf_log( char * mLogMessage, ...)
{
	...
	write( __dpe->iPerfLogHandle, m_buffer, strlen( m_buffer));
...
void dpe__init( char * configfile) 
{
	...
	else
	{
		...
		__dpe->EnableLogs 				= dpdk_config__get_int( config, "EnableLogs", 0, 0);
		__dpe->EnablePerfLogs 			= dpdk_config__get_int( config, "EnablePerfLogs", 0, 0);
		__dpe->EnablePerfLogsTimeSpan 	= dpdk_config__get_int( config, "EnablePerfLogsTimeSpan", 1, 1);
		__dpe->SkipAPICalls				= dpdk_config__get_int( config, "SkipAPICalls", 0, 0);
					...
					for( i = 0; i < count; i++)
					{
						...
						if( interface)
						{
							...
							if(jDestMAC)
							{
								...
								{
									printf("Invalid DestMAC length=%d at interface Index=%d\n", macaddrlen, i);
							...
							if( jtype)
							{
								...
								else if( strcmp( iType, "Gi-Gi") == 0)
								{
									interfaceObject->ProcessType = PROCESS_TYPE__Gi_Gi;
								}
							}
			...
			for( i = 0; i < count; i++)
			{
				...
				if(ipMAC)
				{
					...
					if( jIPv4 && jMAC)
					{
						...
						const char * macaddr = json_string_value( jMAC);
						int macaddrlen = json_string_length( jMAC);
						uint8_t MAC[7];
...
int dpe__get_required_core_count()
{
	...
	while( interface)
	{
		rccount = interface->RecvQueue + interface->SendQueue + interface->RecvWorkerCount + interface->SendWorkerCount + interface->PacketGeneratorCount;
...
int dpe_pkt__encode_gtp_echo_response_ipv4( struct rte_mbuf * mbuf, char * src_mac, char * dst_mac, uint32_t src_ip, uint32_t dst_ip, uint16_t src_port, uint16_t dst_port, uint8_t msgid, uint8_t teid, uint8_t ext, uint8_t type, uint8_t pdu_type, uint8_t qfi)
{
...
int dpe__send_gtp_packet( uint8_t * obj, uint32_t gtpseqno, uint32_t upfteid, uint32_t upfip, uint32_t ueip, uint32_t dstip, uint16_t srcport, uint16_t dstport, uint16_t protocol, uint32_t tcpseqno, char * buffer, int blen)
{
	...
	if( m_buf)
	{
		...
		*/
		...
		rte_memcpy( eth->dst_addr.addr_bytes, mac, 6);
		eth->ether_type = 8;
		...
		//ip_hdr->src_addr 		= rte_cpu_to_be_32( interface->IPv4);
		...
		// pcap_dumper_t * dumper = dpdk_pkt__open_pcap_dump( "./gtppacket.pcap");
...
void dpe__send_arp( dpdk_interface_t * interface, uint32_t tip)
{
	...
	dpdk_pkt__encode_arp_request( m_buf, interface->MacAddr, interface->IPv4, tip);

	// pcap_dumper_t * dumper = dpdk_pkt__open_pcap_dump( "./arp.pcap");
	// dpdk_pkt__write_pcap( dumper, m_buf);
	// dpdk_pkt__close_pcap( dumper);
	// exit(0);
...
struct rte_mbuf * dpe__create_arp( struct rte_mbuf * mbuf, dpdk_interface_t * interface)
{
...
int dpe__isarpformyip( struct rte_mbuf * mbuf_req, uint32_t myip, dpdk_interface_t * interface)
{
	struct rte_ether_hdr * 	request_eth = rte_pktmbuf_mtod( mbuf_req, struct rte_ether_hdr *);
	struct rte_arp_hdr * 	request_arp = (struct rte_arp_hdr *)&request_eth[1];
	...
	if( request_arp->arp_opcode == 512 && request_arp->arp_data.arp_tip == myip /*(Reply to myRequest)*/)
	{
		...
		{
			dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "ARP Response MAC-Addreess %02X:%02X:%02X:%02X:%02X:%02X for IP=%u.%u.%u.%u|%u.%u.%u.%u", 
					request_arp->arp_data.arp_sha.addr_bytes[0] & 0xFF, request_arp->arp_data.arp_sha.addr_bytes[1] & 0xFF,
					request_arp->arp_data.arp_sha.addr_bytes[2] & 0xFF, request_arp->arp_data.arp_sha.addr_bytes[3] & 0xFF,
					request_arp->arp_data.arp_sha.addr_bytes[4] & 0xFF, request_arp->arp_data.arp_sha.addr_bytes[5] & 0xFF,
		...
		else
		{
			...
			while( i < __dpe->arp_table_row_count)
			{
				if( __dpe->arp_table[i].ip == request_arp->arp_data.arp_sip)
				{
					...
					dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "updating MAC=%02X:%02X:%02X:%02X:%02X:%02X for IP=%u", 
			...
			if( bFound == 0 && __dpe->arp_table_row_count < 100)
			{
				...
				// printf("Added MAC=%02X to IP=%u\n", 
					...
					dpe__log( LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "adding MAC=%02X:%02X:%02X:%02X:%02X:%02X for IP=%u", 
						__dpe->arp_table[i].Mac[0] & 0xFF, __dpe->arp_table[i].Mac[1] & 0xFF, __dpe->arp_table[i].Mac[2] & 0xFF, 
						__dpe->arp_table[i].Mac[3] & 0xFF, __dpe->arp_table[i].Mac[4] & 0xFF, __dpe->arp_table[i].Mac[5] & 0xFF, 
					__dpe->arp_table[i].ip);
...
uint16_t dpe__process_received( dpdk_interface_t * interface, struct rte_mbuf ** pkts, uint16_t recv_pkts, struct rte_mbuf ** processed_pkts, struct rte_mbuf ** dropped_mbufs, uint16_t * dropped_cnt_out)
{
	...
	while( i < recv_pkts)
	{
		...
		switch( eth->ether_type)
		{
			...
			case 1544:	//	RTE_ETHER_TYPE_ARP:
				{
					if( dpe__isarpformyip( pkts[i], interface->IPv4, interface) == 1)
					{
						processed_pkts[j] = dpe__create_arp( pkts[i], interface);
						j++;
					}
				}
				break;
...
uint16_t dpe__process_recv_and_send( dpdk_interface_t * interface, struct rte_mbuf ** pkts, uint16_t recv_pkts, struct rte_mbuf ** processed_pkts, struct rte_mbuf ** dropped_mbufs, uint16_t * dropped_cnt_out)
{
	struct rte_ether_hdr * eth = NULL;
	uint16_t dropped_cnt = 0;
...
uint16_t dpe__process_forward( dpdk_interface_t * interface, struct rte_mbuf ** pkts, uint16_t recv_pkts, struct rte_mbuf ** processed_pkts, struct rte_mbuf ** dropped_mbufs, uint16_t * dropped_cnt_out, struct rte_mbuf ** arp_mbufs, uint16_t * arp_cnt_out)
{
	struct rte_ether_hdr * eth = NULL;
	uint16_t dropped_cnt = 0;
	uint16_t arp_cnt = 0;
			...
			case 8:		// RTE_ETHER_TYPE_IPV4:
				{
					...
					{
						// pcap_dumper_t * dumper = dpdk_pkt__open_pcap_dump( "./gtp_packet_1004.pcap");
						...
						if( pt != 1 || ver != 1 || gtp->msg_type != 255)
						{
							dropped_mbufs[dropped_cnt] = pkts[i];
							dropped_cnt++;
							break;
						...
						// pcap_dumper_t * dumper = dpdk_pkt__open_pcap_dump( "./gtp_packet_1304_3.pcap");
						// dpdk_pkt__write_pcap( dumper, pkts[i]);
						// dpdk_pkt__close_pcap( dumper);
						...
						*/
						...
						if( ue_ipv == 4)
						{
							...
							if( cp__flow_allowed_sess_v4( sess, ue_dst_ip, ue_src_port, ue_dst_port, ue_proto, p, pkts[i]->pkt_len, PROCESS_TYPE__GTP_Gi) == 0)
							{
								if( __dpe->EnableLogs == 1)
								{
						...
						// pcap_dumper_t * dumper2 = dpdk_pkt__open_pcap_dump( "./gtp_packet_1004_1.pcap");
					...
					else if( interface->ProcessType == PROCESS_TYPE__Gi_GTP)
					{
						...
						if( __dpe->SkipAPICalls == 0)
						{	
							if( cp__flow_allowed_ueipv4( ue_dst_ip, ue_src_ip, ue_dst_port, ue_src_port, ue_proto, p, pkts[i]->pkt_len, &sess, &p_ran_teid, PROCESS_TYPE__Gi_GTP) == 0)
							{
								if( __dpe->EnableLogs == 1)
								{
									dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "Gi Packet received from ue_ip=%u.%u.%u.%u  dst_ip=%u.%u.%u.%u  src_port=%u  dst_port=%u  protocol=%u  and dropped", 
										ue_dst_ip & 0xFF, (ue_dst_ip >> 8) & 0xFF, (ue_dst_ip >> 16) & 0xFF, (ue_dst_ip >> 24) & 0xFF,
										ue_src_ip & 0xFF, (ue_src_ip >> 8) & 0xFF, (ue_src_ip >> 16) & 0xFF, (ue_src_ip >> 24) & 0xFF,
										ue_src_port, ue_dst_port, ue_proto,
										teid);
							...
							{
								dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "Gi Packet received from ue_ip=%u.%u.%u.%u  dst_ip=%u.%u.%u.%u  src_port=%u  dst_port=%u  protocol=%u  and allowed", 
									ue_dst_ip & 0xFF, (ue_dst_ip >> 8) & 0xFF, (ue_dst_ip >> 16) & 0xFF, (ue_dst_ip >> 24) & 0xFF,
									ue_src_ip & 0xFF, (ue_src_ip >> 8) & 0xFF, (ue_src_ip >> 16) & 0xFF, (ue_src_ip >> 24) & 0xFF,
									ue_src_port, ue_dst_port, ue_proto,
									teid);
						...
						// pcap_dumper_t * dumper2 = dpdk_pkt__open_pcap_dump( "./gi_gtp_packet_in_1304_1.pcap");
						// dpdk_pkt__write_pcap( dumper2, pkts[i]);
						// dpdk_pkt__close_pcap( dumper2);
						// exit(0);
						...
						// pcap_dumper_t * dumper2 = dpdk_pkt__open_pcap_dump( "./gi_gtp_packet_1304_out.pcap");
						// dpdk_pkt__write_pcap( dumper2, pkts[i]);
						// dpdk_pkt__close_pcap( dumper2);
						// exit(0);
...
int dpe__recv_worker( void * args)
{
	...
	while(__dpe->runing)
	{
		...
		else
		{
			...
			{
				//SendAndRecv
				processed_pkts_count = dpe__process_received( interface_port->interface, pkts, recv_pkts, processed_pkts, drop_pkts, &drop_pkts_count);
			}
			else if( __dpe->Type == 2)
			{
				//RecvAndSend
				processed_pkts_count = dpe__process_recv_and_send( interface_port->interface, pkts, recv_pkts, processed_pkts, drop_pkts, &drop_pkts_count);
			}
			else if( __dpe->Type == 3)
			{
				//Forward
				processed_pkts_count = dpe__process_forward( interface_port->interface, pkts, recv_pkts, processed_pkts, drop_pkts, &drop_pkts_count, arp_pkts, &arp_pkts_count);
			}
...
int (*ptrPacketGeneratorFunction)( uint8_t * iobj) = NULL;

void dpe__setPacketGeneratorFunction( int(*ptrPacketGenerator)( uint8_t * iobj))
{
...
int dpe__generator_worker( void * args)
{
	...
	while(__dpe->runing) {
		if( ptrPacketGeneratorFunction > 0) {
			ptrPacketGeneratorFunction( (uint8_t *)interface_port);
		} else {
			usleep( 100000);
		}
	}
...
int dpe__send( void * args)
{
	...
	while(__dpe->runing) 
	{
		...
		else
		{
			...
			// pcap_dumper_t * dumper2 = dpdk_pkt__open_pcap_dump( "./gi_sent_packet_1104.pcap");
			// int z = 0;
			// for( z =0; z < recv_pkts; z++)
			// {
...
void dpe__configure_interface( dpdk_interface_t * interaface)
{
	...
	for( mi = 0; mi < interaface->RecvQueue; mi++)
	{
		...
		interaface->mempool[mi] = rte_pktmbuf_pool_create( mempool_name, interaface->MemPoolSize, 512, 0, RTE_MBUF_DEFAULT_BUF_SIZE, rte_socket_id());
	...
	i = 0;
	for( i = 0; i < interaface->RecvWorkerCount; i++ )
	{
		dpdk_interface_port_t * interaface_port = (dpdk_interface_port_t*)malloc(sizeof(dpdk_interface_port_t));
...
{
	int ret = rte_eth_dev_start( interaface->PortId);
	if (0 != ret)
	{
...
void dpe__add_ranip_to_accessinterface( uint32_t ip)
{
	...
	if( bHasMac == 0)
	{
		...
		while( interaface)
		{
			if( interaface->Type == PROCESS_TYPE__GTP_Gi)
			{
				break;
			}
			interaface = interaface->Next;
...
void * dpe__run_pref_thread( void * args)
{
	...
	while( __dpe->runing)
	{
		...
		while( interaface)
		{
			...
			dpe__perf_log( "Port: %s  r:%8lu %10lu  s:%8lu %10lu   dq:%8lu %10lu   dr:%8lu %10lu   e:%8lu %10lu\n", 
				interaface->Port, received_c, interaface->total_received, sent_c, interaface->total_sent, 
...
void dpe__test__gi_gtp_ipv4_ipv4_icmp()
{
	...
	uint16_t cnt = dpe__process_forward( interface, pkts, recv_pkts, processed_pkts, dropped_mbufs, &dropped_cnt_out, arp_mbufs, &arp_cnt_out);
...
/*
	gcc -g3  eng.c eng_support.c -o eng -march=native -I/usr/local/include -L/usr/local/lib/x86_64-linux-gnu -L/usr/local/lib -L/usr/local/lib/x86_64-linux-gnu -L/usr/local/lib -Wl,--export-dynamic -lmlx5 -libverbs -lIPSec_MB  -lmlx4 -lcrypto -ldl -pthread -lpcap -libverbs -Wl,--as-needed -lrte_node -lrte_graph -lrte_bpf -lrte_flow_classify -lrte_pipeline -lrte_table -lrte_port -lrte_fib -lrte_ipsec -lrte_vhost -lrte_stack -lrte_security -lrte_sched -lrte_reorder -lrte_rib -lrte_regexdev -lrte_rawdev -lrte_pdump -lrte_power -lrte_member -lrte_lpm -lrte_latencystats -lrte_kni -lrte_jobstats -lrte_ip_frag -lrte_gso -lrte_gro -lrte_eventdev -lrte_efd -lrte_distributor -lrte_cryptodev -lrte_compressdev -lrte_cfgfile -lrte_bitratestats -lrte_bbdev -lrte_acl -lrte_timer -lrte_hash -lrte_metrics -lrte_cmdline -lrte_pci -lrte_ethdev -lrte_meter -lrte_net -lrte_mbuf -lrte_mempool -lrte_rcu -lrte_ring -lrte_eal -lrte_telemetry -lrte_kvargs -lisal -lelf -lz -Wl,--as-needed -lrte_node -lrte_graph -lrte_bpf -lrte_flow_classify -lrte_pipeline -lrte_table -lrte_port -lrte_fib -lrte_ipsec -lrte_vhost -lrte_stack -lrte_security -lrte_sched -lrte_reorder -lrte_rib -lrte_regexdev -lrte_rawdev -lrte_pdump -lrte_power -lrte_member -lrte_lpm -lrte_latencystats -lrte_kni -lrte_jobstats -lrte_ip_frag -lrte_gso -lrte_gro -lrte_eventdev -lrte_efd -lrte_distributor -lrte_cryptodev -lrte_compressdev -lrte_cfgfile -lrte_bitratestats -lrte_bbdev -lrte_acl -lrte_timer -lrte_hash -lrte_metrics -lrte_cmdline -lrte_pci -lrte_ethdev -lrte_meter -lrte_net -lrte_mbuf -lrte_mempool -lrte_rcu -lrte_ring -lrte_eal -lrte_telemetry -lrte_kvargs -lm -ldl -lnuma -lfdt -lpcap -lpthread -ljansson
*/
