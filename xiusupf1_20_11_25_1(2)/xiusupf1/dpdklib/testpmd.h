...
//#include <dpi_rte_config.h>
extern dpi_instance * dpi;
extern uint8_t dpi_worker_count;
...
struct fwd_stream {
	...
	// todo: add dpi_lcore id here as param . -1 is a error init value
	int dpi_lcore; //dpi
...
/**
 * @brief dpi_analyze_mbuf dissects the mbuf struct to access the ip_pointer, length and timestamp
 * @param[in] dpi 	is a pointer to an initialized dpi_instance object
 * @param[in] m 	is a pointer to struct ret_mbuf
 * @param[in] tid   every port_id binds a dpi worker instance
 */
static inline void dpi_analyze_mbuf(dpi_instance * const this_dpi, struct rte_mbuf *m, unsigned const tid){

        struct ether_hdr *eth  = rte_pktmbuf_mtod(m, struct ether_hdr *);
        unsigned const eth_len = rte_pktmbuf_pkt_len(m);

        unsigned pid = tid;
        if (dpi_worker_count == 1){
            pid = 0;
        }
        /*check for valid dpi_instance and sufficient frame_len including min. IP_header + UDP_header */
        if (unlikely(this_dpi == NULL || eth_len < (14 + 20 + 8))){
            return;
        }

		// use the external L2 decapsulation for VLAN/MPLS/MAC and GTP-U traffic. DPI needs to inspect the innermost IPhdr
        dpi_return_t ret_dpi = dpi_process_l2_frame(this_dpi, pid, dpi_get_timestamp(DPI_CLOCK_TICKS_PER_SEC), (const void * const)eth, eth_len, DPI_GTPU_ENABLED);
        if (unlikely(ret_dpi != DPI_RETURN_SUCCESS)){
            DEBUG_PRINTF("[ERROR] PACE2 process frame at ts %lu \n", ts);
        }
		// Alternative packet input:
		// if your DPDK application alreadys supports decapsulation, then just pass the innermost IPhdr to dpi_process_packet().
        // struct mac{
        //     uint8_t dst[6];
        //     uint8_t src[6];
        //     uint16_t ethertype;

        // } const *mac_ptr = (struct mac*)eth;

        // if (mac_ptr != NULL &&
        //     (mac_ptr->ethertype == htons(0x0800) ||
        //      mac_ptr->ethertype == htons(0x86DD)) ){

        //     struct iphdr const * const ip = ( struct iphdr*)((uint8_t*)eth + sizeof( struct mac)) ;
        //     uint32_t const ip_len = (eth_len > sizeof(const struct mac)) ? eth_len -  sizeof( struct mac) : eth_len;
        //     DEBUG_PRINTF( "Process frame with ip_len %u on core_id %u\n", ip_len, pid);
		// 	// dpi_process_l2_frame() can be used alternatively if additional L2 tunnel protocols are expected.
        //     dpi_return_t ret_dpi = dpi_process_packet(this_dpi, pid, dpi_get_timestamp(DPI_CLOCK_TICKS_PER_SEC), ip, ip_len);
        //     if (unlikely(ret_dpi != DPI_RETURN_SUCCESS)){
        //         DEBUG_PRINTF("[ERROR] PACE2 process frame at ts %lu \n", ts);
        //     }
        // }
