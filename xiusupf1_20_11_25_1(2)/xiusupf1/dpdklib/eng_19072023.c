...
int dpe__get_required_core_count()
{
	...
	while( interface)
	{
		rccount = interface->RecvQueue + interface->SendQueue + interface->RecvWorkerCount + interface->SendWorkerCount;
			...
			case 8:		// RTE_ETHER_TYPE_IPV4:
				{
					...
					else if( interface->ProcessType == PROCESS_TYPE__Gi_GTP)
					{
						...
						{	
							...
							{
								dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "Gi Packet received from ue_ip=%u.%u.%u.%u  dst_ip=%u.%u.%u.%u  src_port=%u  dst_port=%u  protocol=%u  and allowed UPF-TEID=%u  RAN-TEID=%u", 
									ue_dst_ip & 0xFF, (ue_dst_ip >> 8) & 0xFF, (ue_dst_ip >> 16) & 0xFF, (ue_dst_ip >> 24) & 0xFF,
									ue_src_ip & 0xFF, (ue_src_ip >> 8) & 0xFF, (ue_src_ip >> 16) & 0xFF, (ue_src_ip >> 24) & 0xFF,
									ue_src_port, ue_dst_port, ue_proto,
									teid, p_ran_teid);
						...
						if( __dpe->EnableLogs == 1)
						{
							dpe__log( LOG_LEVEL_DEBUG, __FILE__, __LINE__, "Gi Packet, Adding GTP Layer for  UPF-TEID=%u  RAN-TEID=%u   RAN-UPv4=%u.%u.%u.%u  RAN-MAC=%02X:%02X:%02X:%02X:%02X:%02X ", 
								teid, p_ran_teid, p_ran_ip_v4 & 0xFF, (p_ran_ip_v4 >> 8) & 0xFF, (p_ran_ip_v4 >> 16) & 0xFF, (p_ran_ip_v4 >> 24) & 0xFF,
								ranmac[0] & 0xFF, ranmac[1] & 0xFF, ranmac[2] & 0xFF, ranmac[3] & 0xFF, ranmac[4] & 0xFF, ranmac[5] & 0xFF
								);
						}
...
void dpe__add_ranip_to_accessinterface( uint32_t ip)
{
	...
	if( bFound == 0)
	{
		...
		if( bFound == 0 && __dpe->arp_table_row_count < 50)
		{
