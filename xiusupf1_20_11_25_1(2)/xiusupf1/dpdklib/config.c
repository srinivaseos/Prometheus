...
setup_fwd_config_of_each_lcore(struct fwd_config *cfg)
{
	...
	dpi_worker_count = nb_fc;
	dpi_optargs_t dpi_args ={0}; //todo expose args in testpmd or via command
	dpi_read_configfile(getenv("DPI_CFG"), &dpi_args);
	dpi_return_t ret_dpi = dpi_init_instance(&dpi, dpi_worker_count, &dpi_args);

	if (ret_dpi != DPI_RETURN_SUCCESS){
        fprintf(stdout, "[ERROR] at DPI init global\n");
        return;
    } else {
        fprintf(stdout, "[INFO] DPI instance pre-init for %u worker instances\n", dpi_worker_count);
        uint16_t const ver_str_len = 64;
        char p2v[ver_str_len];
        if ( DPI_RETURN_SUCCESS == dpi_get_dpi_version(dpi, 0, p2v, ver_str_len) ){
            fprintf(stdout, "[INFO] DPI version %s initialized\n", p2v);
        }
    }
	// todo: remove, dpi add better readability
